#!/usr/bin/env python3
"""Finalize the assembled tree as a versioned v0.25.2 candidate package."""
...
CANDIDATE_ARCHIVE_RE = re.compile(
    ...
    r"(?:-(?P<source>[0-9a-f]{7}))?-v0252\.tar\.gz$"
)
...
STATUS_RELATIVE_PATH = "docs/v0252-candidate-status.json"
TARGET_ACCEPTANCE_RELATIVE_PATH = "docs/v0252-target-machine-acceptance.md"
...
def _basic_information_bounds(content: str) -> Tuple[int, int]:
    ...
    if basic_index is None:
        raise ValueError("V0252_TARGET_ACCEPTANCE_BASIC_INFO_SECTION_MISSING")
    next_index = next(
        (
            index
...
def _validate_basic_info_schema(content: str, context_lines: List[str]) -> None:
    ...
    if lines.count(CANONICAL_DOCUMENT_TITLE) != 1:
        raise ValueError("V0252_TARGET_ACCEPTANCE_DOCUMENT_TITLE_CARDINALITY_INVALID")
    if lines.count(CANONICAL_BASIC_INFO_HEADING) != 1:
        raise ValueError("V0252_TARGET_ACCEPTANCE_BASIC_INFO_HEADING_CARDINALITY_INVALID")
    if lines.count(CANONICAL_NEXT_HEADING) != 1:
        raise ValueError("V0252_TARGET_ACCEPTANCE_NEXT_HEADING_CARDINALITY_INVALID")
    if lines.count(CANDIDATE_CONTEXT_BEGIN) != 1 or lines.count(CANDIDATE_CONTEXT_END) != 1:
        raise ValueError("V0252_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_INVALID")
    if len(lines) < len(expected) or lines[: len(expected)] != expected:
        raise ValueError("V0252_TARGET_ACCEPTANCE_BASIC_INFO_SCHEMA_MISMATCH")
...
def _validate_archive_source_binding(
    ...
    if (
        ...
        raise ValueError("V0252_PREVIOUS_CANDIDATE_SOURCE_BINDING_INVALID")


def candidate_archive_name(date_tag: str, source_commit: str) -> str:
    return "ai-wps-phase1-delivery-{0}-{1}-v0252.tar.gz".format(
...
def baseline_metadata(archive: Path, expected_version: str) -> dict:
    if not archive.is_file():
        raise ValueError("V0251_BASELINE_ARCHIVE_MISSING")
    ...
    try:
        with tarfile.open(str(archive), "r:gz") as handle:
            ...
            if extracted is None:
                raise ValueError("V0251_BASELINE_MANIFEST_UNREADABLE")
            manifest = json.loads(extracted.read().decode("utf-8"))
    except (OSError, tarfile.TarError, StopIteration, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("V0251_BASELINE_ARCHIVE_INVALID") from exc
    if manifest.get("version") != expected_version:
        raise ValueError("V0251_BASELINE_VERSION_INVALID")
    if manifest.get("deliveryPolicy", {}).get("status") != "candidate":
        raise ValueError("V0251_BASELINE_NOT_CANDIDATE")
    return {
...
def previous_candidate_metadata(archive: Path) -> dict:
    ...
    if not archive.is_file():
        raise ValueError("V0252_PREVIOUS_CANDIDATE_ARCHIVE_MISSING")
    if match is None:
        raise ValueError("V0252_PREVIOUS_CANDIDATE_ARCHIVE_NAME_INVALID")
    digest = hashlib.sha256(archive.read_bytes()).hexdigest()
    try:
        with tarfile.open(str(archive), "r:gz") as handle:
            member = next(
            ...
            extracted = handle.extractfile(member)
            if extracted is None:
                raise ValueError("V0252_PREVIOUS_CANDIDATE_MANIFEST_UNREADABLE")
            manifest = json.loads(extracted.read().decode("utf-8"))
    except (OSError, tarfile.TarError, StopIteration, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("V0252_PREVIOUS_CANDIDATE_ARCHIVE_INVALID") from exc
    previous_version = manifest.get("version")
    if previous_version not in (VERSION, BASELINE_VERSION):
        raise ValueError("V0252_PREVIOUS_CANDIDATE_VERSION_INVALID")
    if manifest.get("releaseDate") != match.group("date"):
        raise ValueError("V0252_PREVIOUS_CANDIDATE_DATE_MISMATCH")
    if manifest.get("deliveryPolicy", {}).get("status") != "candidate":
        raise ValueError("V0252_PREVIOUS_CANDIDATE_NOT_CANDIDATE")
    ...
    if not isinstance(build_id, str) or not PREVIOUS_BUILD_ID_RE.fullmatch(build_id):
        raise ValueError("V0252_PREVIOUS_CANDIDATE_BUILD_ID_MISSING")
...
def record_candidate_lineage(
    ...
    if not path.is_file():
        raise ValueError("V0252_CANDIDATE_STATUS_MISSING")
    ...
    if (
        ...
        raise ValueError("V0252_CANDIDATE_STATUS_INVALID")
    previous_records = [
        item
    ...
    if previous.get("productVersion") == BASELINE_VERSION:
        if previous_records:
            raise ValueError("V0252_PREVIOUS_PRODUCT_CANDIDATE_MUST_NOT_ENTER_STATUS")
    else:
        if len(previous_records) != 1:
            raise ValueError("V0252_PREVIOUS_CANDIDATE_STATUS_MISSING")
        ...
        if (
            ...
            raise ValueError("V0252_PREVIOUS_CANDIDATE_STATUS_INVALID")
        if previous.get("sourceCommit") and (
            previous_record.get("sourceCommit") != previous["sourceCommit"]
        ):
            raise ValueError("V0252_PREVIOUS_CANDIDATE_STATUS_SOURCE_COMMIT_INVALID")
    current_record = {
...
def write_candidate_delivery_note(
    ...
    note_path = root / "docs/v0252-delivery.md"
    if not note_path.parent.is_dir():
        raise ValueError("V0252_CANDIDATE_NOTE_DIRECTORY_MISSING")
    note = """# v0.25.2-alpha candidate delivery note

This file is generated by `prepare_v0252_delivery.py` from the assembled
...
The package uses the accepted `v0.25.1-alpha` Phase1 baseline and explicit
...
Overlay upgrades from 0.25.1 migrate existing direct configs into the same
...
def write_target_machine_acceptance_record(
    ...
    if not record_path.is_file():
        raise ValueError("V0252_TARGET_ACCEPTANCE_RECORD_MISSING")
    ...
    for marker in TARGET_ACCEPTANCE_MATRIX_MARKERS:
        if marker not in content:
            raise ValueError("V0252_TARGET_ACCEPTANCE_MATRIX_MISSING {0}".format(marker))
    ...
    if previous.get("productVersion") == BASELINE_VERSION:
        previous_line = (
            ...
            "candidateBuildId：`{candidate_build_id}`，状态仍为 `{status}`，不是 0.25.2 当前候选"
        ).format(
    ...
    if begin > end:
        raise ValueError("V0252_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_ORDER_INVALID")
    ...
    if begin < section_start or end + len(CANDIDATE_CONTEXT_END) > section_end:
        raise ValueError("V0252_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_OUTSIDE_BASIC_INFO")
...
def prepare(
    ...
    if baseline_version != BASELINE_VERSION:
        raise ValueError("V0251_BASELINE_VERSION_REQUIRED")
    if not date_tag or len(date_tag) != 8 or not date_tag.isdigit():
        raise ValueError("V0252_DATE_INVALID")
    try:
        datetime.strptime(date_tag, "%Y%m%d")
    except ValueError as exc:
        raise ValueError("V0252_DATE_INVALID") from exc
    if not SOURCE_COMMIT_RE.fullmatch(source_commit):
        raise ValueError("V0252_SOURCE_COMMIT_INVALID")
    ...
    if previous["archiveName"] == current_archive_name:
        raise ValueError("V0252_PREVIOUS_CANDIDATE_BUILD_REUSE")
    rewrite_versions(root, ("0.23.1-alpha", "0.24.0-alpha", "0.25.0-alpha", "0.25.1-alpha"))
    ...
    manifest["versionRule"] = "AI-WPS-P1-WORD-EXCEL-PPT-0.25.2-" + date_tag
    ...
    manifest["deliveryPolicy"] = {
        ...
        "candidateAuditScript": "scripts/audit_v0252_delivery.py",
    ...
    print("v0252_delivery_prepared=passed version={0}".format(VERSION))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        print("v0252_delivery_prepared=failed {0}".format(exc))
        return 1
    return 0
