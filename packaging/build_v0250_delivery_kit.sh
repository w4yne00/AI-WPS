...
SOURCE_ALLOWLIST="$ROOT_DIR/packaging/delivery-sources-v0250.json"
PUBLISHED_ARCHIVE="0"
...
if [ -z "$BASELINE_ARCHIVE" ] || [ ! -f "$BASELINE_ARCHIVE" ]; then
  echo "v0240_baseline_archive_required=true"
  exit 1
fi
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/prepare_v0250_delivery.py" \
  "$TMP_DIR" \
  --date "$DATE_TAG" \
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/check_python38_compatibility.py" \
  ...
  "$TMP_DIR/scripts/audit_v0250_delivery.py" \
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/audit_v0250_delivery.py" "$TMP_DIR"
...
PUBLISHED_ARCHIVE="0"
echo "v0250_delivery_candidate=$ARCHIVE_PATH status=candidate"
trap - EXIT
