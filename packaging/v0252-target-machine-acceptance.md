...
<!-- V0252-CANDIDATE-CONTEXT:BEGIN -->
- 当前源树没有活动的 0.25.2 候选；冻结的 `v0.25.1-alpha` 候选 `d7a1dd8` 保持原字节，SHA-256 为 `ec318db4ffbda499c24aa6fb50958628cc4eaa030b22389bbf29cd783b1adbf6`，状态仍为 `candidate`，不是 0.25.2 当前候选。
<!-- V0252-CANDIDATE-CONTEXT:END -->
- 验收人员：
- 验收日期：
- 交付包文件名：
- 交付包 SHA-256：
...
本记录是目标机现场填写模板。候选上下文由 `prepare_v0252_delivery.py` 在组装交付树时整体生成；自动化门禁只能证明候选构建，不能替代麒麟 V10、目标 WPS 和 `cloud` 用户环境中的真实操作。所有现场原始命令输出、截图、目标机编号、账号信息、配置内容、API Key、文档正文和模型原始回复只保留在受控验收记录中，不写入仓库。
