...
`ai-wps-phase1-delivery-<YYYYMMDD>-<SOURCE_COMMIT>-v0251.tar.gz`。其中 `SOURCE_COMMIT` 为七位源码提交号，使同一天的修复重建仍具有独立、可追溯的归档身份；旧的无提交号归档名继续作为历史候选被接受。它只收敛格式审查的正文大纲级别、标题定位、正式模型直连验证和运行时调用诊断，不进入 Preview 安装基线，也不改变 Phase1 的安装入口、目录或回退事务。
