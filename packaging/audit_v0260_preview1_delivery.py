    ...
    probe = subprocess.run(
        [
            ...
            (
                ...
                "cap_validated={'status':'validated','serviceId':'direct_svc_test',"
                ...
                "cap_not_checked={'status':'not_checked'}; "
                "cap_unsupported={'status':'unsupported'}; "
                "make_worker=lambda cap:type('Worker',(),{"
                "'snapshot_task_auth':lambda self,cap=cap:{'streamingCapability':cap},"
                ...
                "job=SmartWriteJobStore(worker=make_worker(cap_validated),coordinator=coordinator).start("
                "request,'audit-trace'); "
                ...
                "payload['clientJobId']='audit-job-not-checked'; "
                "request=(WordDocumentRequest.model_validate(payload) "
                "if hasattr(WordDocumentRequest,'model_validate') "
                "else WordDocumentRequest.parse_obj(payload)); "
                "coordinator=LongTaskCoordinator(max_running=1,max_queued=1); "
                "job=SmartWriteJobStore(worker=make_worker(cap_not_checked),coordinator=coordinator).start("
                "request,'audit-not-checked'); "
                "assert job.get('streamingEnabled') is False; "
                "stored=coordinator._jobs[('word.smart_write',job['jobId'])]; "
                "assert stored['_allowRunningCancel'] is False; "
                "payload['clientJobId']='audit-job-unsupported'; "
                "request=(WordDocumentRequest.model_validate(payload) "
                "if hasattr(WordDocumentRequest,'model_validate') "
                "else WordDocumentRequest.parse_obj(payload)); "
                "coordinator=LongTaskCoordinator(max_running=1,max_queued=1); "
                "job=SmartWriteJobStore(worker=make_worker(cap_unsupported),coordinator=coordinator).start("
                "request,'audit-unsupported'); "
                "assert job.get('streamingEnabled') is False; "
                "stored=coordinator._jobs[('word.smart_write',job['jobId'])]; "
                "assert stored['_allowRunningCancel'] is False; "
                "sent=[]; "
                ...
                "'read':lambda self,*args:b'{\"choices\":[{\"message\":{\"content\":\"ok\"},\"finish_reason\":\"stop\"}]}'})(); "
                "fake=lambda req,timeout=None:(sent.append(req) or response); "
                "auth={'providerBaseUrl':'https://api.example.test/v1','apiKey':'test',"
                "'modelName':'model-test','streamingCapability':cap_not_checked,"
                "'directStreamingEnabled':True,'contextWindowTokens':100000,"
                "'maxOutputTokens':128}; "
                "mock=patch('app.services.provider_client.urllib_request.urlopen',side_effect=fake); "
                "mock.start(); "
                "ProviderClient()._post_direct_task('word.smart_write','audit-provider',"
                "'audit',auth,30,prompt_asset={'content':'audit','version':'v1','hashPrefix':'hash'}); "
                "mock.stop(); "
                "assert json.loads(sent[0].data.decode('utf-8'))['stream'] is False"
            ),
        ],
        cwd=adapter_root,
        env=probe_env,
