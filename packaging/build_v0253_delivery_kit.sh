...
BASELINE_ARCHIVE="${AI_WPS_V0252_BASELINE_ARCHIVE:-}"
PREVIOUS_CANDIDATE_ARCHIVE="${AI_WPS_V0253_PREVIOUS_CANDIDATE_ARCHIVE:-${AI_WPS_V0252_BASELINE_ARCHIVE:-}}"
...
KIT_NAME="ai-wps-phase1-delivery-${DATE_TAG}-${SOURCE_TAG}-v0253"
...
SOURCE_ALLOWLIST="$ROOT_DIR/packaging/delivery-sources-v0253.json"
PUBLISHED_ARCHIVE="0"
...
if [ -z "$BASELINE_ARCHIVE" ] || [ ! -f "$BASELINE_ARCHIVE" ]; then
  echo "v0252_baseline_archive_required=true"
  exit 1
fi
if [ -z "$PREVIOUS_CANDIDATE_ARCHIVE" ] || [ ! -f "$PREVIOUS_CANDIDATE_ARCHIVE" ]; then
  echo "v0253_previous_candidate_archive_required=true"
  exit 1
fi
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/prepare_v0253_delivery.py" \
  "$TMP_DIR" \
  --date "$DATE_TAG" \
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/check_python38_compatibility.py" \
  ...
  "$TMP_DIR/scripts/audit_v0253_delivery.py" \
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/audit_v0253_delivery.py" "$TMP_DIR"
"$PYTHON_BIN" "$TMP_DIR/scripts/audit_v0253_delivery.py" "$TMP_DIR"
...
PUBLISHED_ARCHIVE="0"
echo "v0253_delivery_candidate=$ARCHIVE_PATH status=candidate source_commit=$SOURCE_COMMIT"
trap - EXIT
