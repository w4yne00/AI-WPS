#!/usr/bin/env python3
"""Finalize the assembled tree as a versioned v0.25.3 candidate package."""
...
CANDIDATE_ARCHIVE_RE = re.compile(
    ...
    r"(?:-(?P<source>[0-9a-f]{7}))?-v0253\.tar\.gz$"
)
PREVIOUS_ARCHIVE_RE = re.compile(
    r"^ai-wps-phase1-delivery-(?P<date>[0-9]{8})"
...
VERSION_REWRITE_EXCLUDED_PATHS = {
    "README.md",
    "docs/v0253-delivery.md",
    "docs/v0253-target-machine-acceptance.md",
    "docs/v0253-candidate-status.json",
    "scripts/audit_delivery.py",
    "scripts/audit_v0253_delivery.py",
    "scripts/python38_delivery_lifecycle_gate.py",
    "scripts/python38_delivery_runtime_gate.py",
}
STATUS_RELATIVE_PATH = "docs/v0253-candidate-status.json"
TARGET_ACCEPTANCE_RELATIVE_PATH = "docs/v0253-target-machine-acceptance.md"
...
def _basic_information_bounds(content: str) -> Tuple[int, int]:
    ...
    if basic_index is None:
        raise ValueError("V0253_TARGET_ACCEPTANCE_BASIC_INFO_SECTION_MISSING")
    next_index = next(
        (
            index
...
def _validate_basic_info_schema(content: str, context_lines: List[str]) -> None:
    ...
    if lines.count(CANONICAL_DOCUMENT_TITLE) != 1:
        raise ValueError("V0253_TARGET_ACCEPTANCE_DOCUMENT_TITLE_CARDINALITY_INVALID")
    if lines.count(CANONICAL_BASIC_INFO_HEADING) != 1:
        raise ValueError("V0253_TARGET_ACCEPTANCE_BASIC_INFO_HEADING_CARDINALITY_INVALID")
    if lines.count(CANONICAL_NEXT_HEADING) != 1:
        raise ValueError("V0253_TARGET_ACCEPTANCE_NEXT_HEADING_CARDINALITY_INVALID")
    if lines.count(CANDIDATE_CONTEXT_BEGIN) != 1 or lines.count(CANDIDATE_CONTEXT_END) != 1:
        raise ValueError("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_INVALID")
    if len(lines) < len(expected) or lines[: len(expected)] != expected:
        raise ValueError("V0253_TARGET_ACCEPTANCE_BASIC_INFO_SCHEMA_MISMATCH")
...
def _validate_archive_source_binding(
    ...
    if (
        ...
        or not build_id.startswith(
            "AI-WPS-P1-WORD-EXCEL-PPT-0.25.{0}-{1}-".format(series, match.group("date"))
        )
    ):
        raise ValueError("V0253_PREVIOUS_CANDIDATE_SOURCE_BINDING_INVALID")


def candidate_archive_name(date_tag: str, source_commit: str) -> str:
    return "ai-wps-phase1-delivery-{0}-{1}-v0253.tar.gz".format(
...
def baseline_metadata(archive: Path, expected_version: str) -> dict:
    if not archive.is_file():
        raise ValueError("V0253_BASELINE_ARCHIVE_MISSING")
    ...
    try:
        with tarfile.open(str(archive), "r:gz") as handle:
            ...
            if extracted is None:
                raise ValueError("V0253_BASELINE_MANIFEST_UNREADABLE")
            manifest = json.loads(extracted.read().decode("utf-8"))
    except (OSError, tarfile.TarError, StopIteration, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("V0253_BASELINE_ARCHIVE_INVALID") from exc
    if manifest.get("version") != expected_version:
        raise ValueError("V0253_BASELINE_VERSION_INVALID")
    if manifest.get("deliveryPolicy", {}).get("status") != "candidate":
        raise ValueError("V0253_BASELINE_NOT_CANDIDATE")
    return {
...
def previous_candidate_metadata(archive: Path) -> dict:
    match = PREVIOUS_ARCHIVE_RE.fullmatch(archive.name)
    if not archive.is_file():
        raise ValueError("V0253_PREVIOUS_CANDIDATE_ARCHIVE_MISSING")
    if match is None:
        raise ValueError("V0253_PREVIOUS_CANDIDATE_ARCHIVE_NAME_INVALID")
    digest = hashlib.sha256(archive.read_bytes()).hexdigest()
    try:
        with tarfile.open(str(archive), "r:gz") as handle:
            member = next(
            ...
            extracted = handle.extractfile(member)
            if extracted is None:
                raise ValueError("V0253_PREVIOUS_CANDIDATE_MANIFEST_UNREADABLE")
            manifest = json.loads(extracted.read().decode("utf-8"))
    except (OSError, tarfile.TarError, StopIteration, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("V0253_PREVIOUS_CANDIDATE_ARCHIVE_INVALID") from exc
    previous_version = manifest.get("version")
    if previous_version not in (VERSION, BASELINE_VERSION):
        raise ValueError("V0253_PREVIOUS_CANDIDATE_VERSION_INVALID")
    if manifest.get("releaseDate") != match.group("date"):
        raise ValueError("V0253_PREVIOUS_CANDIDATE_DATE_MISMATCH")
    if manifest.get("deliveryPolicy", {}).get("status") != "candidate":
        raise ValueError("V0253_PREVIOUS_CANDIDATE_NOT_CANDIDATE")
    ...
    if not isinstance(build_id, str) or not PREVIOUS_BUILD_ID_RE.fullmatch(build_id):
        raise ValueError("V0253_PREVIOUS_CANDIDATE_BUILD_ID_MISSING")
    ...
    return {
        ...
        "status": "candidate" if previous_version == BASELINE_VERSION else "rejected",
        "productVersion": previous_version,
    }
...
def record_candidate_lineage(
    ...
    if not path.is_file():
        raise ValueError("V0253_CANDIDATE_STATUS_MISSING")
    ...
    if (
        ...
        raise ValueError("V0253_CANDIDATE_STATUS_INVALID")
    previous_records = [
        item
        for item in status["records"]
        if isinstance(item, dict) and item.get("archiveName") == previous["archiveName"]
    ]
    if previous.get("productVersion") == BASELINE_VERSION:
        if previous_records:
            raise ValueError("V0253_PREVIOUS_PRODUCT_CANDIDATE_MUST_NOT_ENTER_STATUS")
    else:
        if len(previous_records) != 1:
            raise ValueError("V0253_PREVIOUS_CANDIDATE_STATUS_MISSING")
        ...
        if (
            ...
            raise ValueError("V0253_PREVIOUS_CANDIDATE_STATUS_INVALID")
        if previous.get("sourceCommit") and (
            previous_record.get("sourceCommit") != previous["sourceCommit"]
        ):
            raise ValueError("V0253_PREVIOUS_CANDIDATE_STATUS_SOURCE_COMMIT_INVALID")
    current_record = {
...
def remove_previous_candidate_identity(root: Path) -> None:
    old_paths = {
        "docs/v0252-delivery.md",
        "docs/v0252-target-machine-acceptance.md",
        "docs/v0252-candidate-status.json",
        "scripts/audit_v0252_delivery.py",
    }
...
def write_candidate_delivery_note(
    ...
    note_path = root / "docs/v0253-delivery.md"
    if not note_path.parent.is_dir():
        raise ValueError("V0253_CANDIDATE_NOTE_DIRECTORY_MISSING")
    note = """# v0.25.3-alpha candidate delivery note

This file is generated by `prepare_v0253_delivery.py` from the assembled
...
The package uses the accepted `v0.25.2-alpha` Phase1 baseline and explicit
...
New installs default-enable 图像语义补充 for format-review direct models.
Overlay upgrades from 0.25.2 migrate existing direct configs into the same
outbound default. Visual capability failure or a closed master switch is
...
emoji checks run only through the v2 snapshot/batch/job backend path.
...
def write_target_machine_acceptance_record(
    ...
    if not record_path.is_file():
        raise ValueError("V0253_TARGET_ACCEPTANCE_RECORD_MISSING")
    ...
    for marker in TARGET_ACCEPTANCE_MATRIX_MARKERS:
        if marker not in content:
            raise ValueError("V0253_TARGET_ACCEPTANCE_MATRIX_MISSING {0}".format(marker))
    ...
    if begin > end:
        raise ValueError("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_ORDER_INVALID")
    ...
    if begin < section_start or end + len(CANDIDATE_CONTEXT_END) > section_end:
        raise ValueError("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_OUTSIDE_BASIC_INFO")
...
def prepare(
    ...
    if baseline_version != BASELINE_VERSION:
        raise ValueError("V0253_BASELINE_VERSION_REQUIRED")
    if not date_tag or len(date_tag) != 8 or not date_tag.isdigit():
        raise ValueError("V0253_DATE_INVALID")
    try:
        datetime.strptime(date_tag, "%Y%m%d")
    except ValueError as exc:
        raise ValueError("V0253_DATE_INVALID") from exc
    if not SOURCE_COMMIT_RE.fullmatch(source_commit):
        raise ValueError("V0253_SOURCE_COMMIT_INVALID")
    ...
    if previous["archiveName"] == current_archive_name:
        raise ValueError("V0253_PREVIOUS_CANDIDATE_BUILD_REUSE")
    rewrite_versions(root, ("0.23.1-alpha", "0.24.0-alpha", "0.25.0-alpha", "0.25.1-alpha", "0.25.2-alpha"))
    ...
    manifest["versionRule"] = "AI-WPS-P1-WORD-EXCEL-PPT-0.25.3-" + date_tag
    ...
    manifest["deliveryPolicy"] = {
        ...
        "candidateAuditScript": "scripts/audit_v0253_delivery.py",
    ...
    print("v0253_delivery_prepared=passed version={0}".format(VERSION))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        print("v0253_delivery_prepared=failed {0}".format(exc))
        return 1
    return 0
