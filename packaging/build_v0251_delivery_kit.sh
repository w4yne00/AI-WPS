...
"$PYTHON_BIN" "$ROOT_DIR/packaging/prepare_v0251_delivery.py" \
  "$TMP_DIR" \
  --date "$DATE_TAG" \
...
"$PYTHON_BIN" "$ROOT_DIR/packaging/check_python38_compatibility.py" \
  ...
  "$TMP_DIR/scripts/audit_v0251_delivery.py" \
...
"$PYTHON_BIN" "$TMP_DIR/scripts/audit_v0251_delivery.py" "$TMP_DIR"
...
PUBLISHED_ARCHIVE="0"
echo "v0251_delivery_candidate=$ARCHIVE_PATH status=candidate source_commit=$SOURCE_COMMIT"
trap - EXIT
