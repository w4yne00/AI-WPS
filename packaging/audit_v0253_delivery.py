#!/usr/bin/env python3
"""Audit the assembled v0.25.3-alpha Phase1 candidate."""
...
BUILD_ID_RE = re.compile(
    r"^AI-WPS-P1-WORD-EXCEL-PPT-0\.25\.3-[0-9]{8}-[0-9a-f]{7,40}$"
)
PREVIOUS_BUILD_ID_RE = re.compile(
    r"^AI-WPS-P1-WORD-EXCEL-PPT-0\.25\.[123]-[0-9]{8}(?:-[0-9a-f]{7,40})?$"
)
REFERENCE_WORKFLOWS = {
...
CANDIDATE_CONTEXT_BEGIN = "<!-- V0253-CANDIDATE-CONTEXT:BEGIN -->"
CANDIDATE_CONTEXT_END = "<!-- V0253-CANDIDATE-CONTEXT:END -->"
CANONICAL_DOCUMENT_TITLE = "# v0.25.3 目标机整合验收记录"
...
CANONICAL_BASIC_INFO_INTRO = (
    "本记录是目标机现场填写模板。候选上下文由 `prepare_v0253_delivery.py` 在组装交付树时整体生成；"
...
CANONICAL_SOURCE_CONTEXT_LINE = (
    "- 当前源树没有活动的 0.25.3 候选；冻结的 `v0.25.2-alpha` 候选 `850871c` 保持原字节，"
    "SHA-256 为 `c5d663d1249147104bee66790fea60f5e15675418a51c0c1a7a0fc028a285a92`，"
    "状态仍为 `candidate`，不是 0.25.3 当前候选。"
)
...
def _basic_information_bounds(content: str) -> tuple:
    ...
    if basic_index is None:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_BASIC_INFO_SECTION_MISSING")
    next_index = next(
        (
            index
...
def _validate_basic_info_schema(content: str, context_lines: List[str]) -> None:
    ...
    if lines.count(CANONICAL_DOCUMENT_TITLE) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_DOCUMENT_TITLE_CARDINALITY_INVALID")
    if lines.count(CANONICAL_BASIC_INFO_HEADING) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_BASIC_INFO_HEADING_CARDINALITY_INVALID")
    if lines.count(CANONICAL_NEXT_HEADING) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_NEXT_HEADING_CARDINALITY_INVALID")
    if lines.count(CANDIDATE_CONTEXT_BEGIN) != 1 or lines.count(CANDIDATE_CONTEXT_END) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_INVALID")
    if len(lines) < len(expected) or lines[: len(expected)] != expected:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_BASIC_INFO_SCHEMA_MISMATCH")
...
def audit_archive_checksum(
    ...
    if not archive.is_file() or not checksum_file.is_file():
        raise DeliveryFailure("V0253_ARCHIVE_CHECKSUM_FILE_MISSING")
    fields = checksum_file.read_text(encoding="utf-8").strip().split()
    if len(fields) != 2 or not SHA256_RE.fullmatch(fields[0]):
        raise DeliveryFailure("V0253_ARCHIVE_CHECKSUM_FORMAT_INVALID")
    if fields[1] != (expected_archive_name or archive.name):
        raise DeliveryFailure("V0253_ARCHIVE_CHECKSUM_NAME_MISMATCH")
    if fields[0] != sha256(archive):
        raise DeliveryFailure("V0253_ARCHIVE_CHECKSUM_MISMATCH")
...
def audit_candidate_note(root: Path, manifest: Dict) -> None:
    ...
    note = safe_path(root, "docs/v0253-delivery.md", "V0253_CANDIDATE_NOTE_MISSING")
    content = note.read_text(encoding="utf-8")
    for pattern in STALE_CANDIDATE_NOTE_PATTERNS:
        if pattern.search(content):
            raise DeliveryFailure("V0253_CANDIDATE_NOTE_STALE {0}".format(pattern.pattern))
    ...
    archive_name = "ai-wps-phase1-delivery-{0}-{1}-v0253.tar.gz".format(
    ...
    for marker in required:
        if marker not in content:
            raise DeliveryFailure("V0253_CANDIDATE_NOTE_IDENTITY_MISSING {0}".format(marker))
...
def _candidate_context(content: str) -> str:
    ...
    if begin_count == 0 or end_count == 0:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_MISSING")
    if begin_count != 1 or end_count != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_DUPLICATE")
    begin = begin_positions[0]
    end = end_positions[0]
    if begin > end:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_DELIMITER_ORDER_INVALID")
    ...
    if (
        ...
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_OUTSIDE_BASIC_INFO")
...
def _validate_candidate_context(content: str, manifest: Dict) -> None:
    ...
    if any(pattern.search(basic_section) for pattern in CANDIDATE_CONTEXT_STALE_PATTERNS):
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_STALE_NARRATIVE")
    lines = _non_empty_lines(context)
    current_lines = [line for line in lines if line.startswith("- 当前自动化候选：")]
    if not current_lines:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CURRENT_CANDIDATE_LINE_MISSING")
    if len(current_lines) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CURRENT_CANDIDATE_LINE_DUPLICATE")
    ...
    if current_lines[0] != expected_current:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_IDENTITY_MISMATCH")

    previous = evidence.get("supersedes", {})
    if previous.get("status") == "candidate":
        expected_previous = (
            "- 0.25.2 产品候选：`{archive_name}`，SHA-256：`{archive_sha256}`；"
            "candidateBuildId：`{candidate_build_id}`，状态仍为 `candidate`，不是 0.25.3 当前候选"
        ).format(
    ...
    if not previous_lines:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_SUPERSEDED_LINE_MISSING")
    if len(previous_lines) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_SUPERSEDED_LINE_DUPLICATE")
    if previous_lines[0] != expected_previous:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_SUPERSEDED_IDENTITY_MISMATCH")

    state_lines = [line for line in lines if line.startswith("- 候选状态：")]
    if not state_lines:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_STATE_MISSING")
    if len(state_lines) != 1:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_STATE_DUPLICATE")
    expected_state = (
        "- 候选状态：当前归档是自动化门禁产生的当前候选；"
        "自动化门禁不等于目标机验收，Issue #59 仍为 `manual-pending`。"
    )
    if state_lines[0] != expected_state:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_STATE_MISMATCH")

    expected_lines = [expected_current, expected_previous, expected_state]
    if any(line not in expected_lines for line in lines):
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_UNKNOWN_LINE")
    if len(lines) != len(expected_lines):
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_LINE_CARDINALITY_INVALID")
    if lines != expected_lines:
        raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_CANDIDATE_CONTEXT_ORDER_INVALID")
...
def audit_target_acceptance_record(root: Path, manifest: Dict) -> None:
    ...
    extra_rows = sorted(
        {index for index, _status in numbered_rows if index not in set(range(1, 13))}
    )
    if extra_rows:
    ...
    missing_rows = set(range(1, 13)) - set(result_rows)
    if missing_rows:
        raise DeliveryFailure(
    ...
    for marker in TARGET_ACCEPTANCE_MATRIX_MARKERS:
        if marker not in content:
            raise DeliveryFailure("V0253_TARGET_ACCEPTANCE_MATRIX_MISSING {0}".format(marker))

    status = load_json(
        root / "docs/v0253-candidate-status.json",
        "V0253_CANDIDATE_STATUS_MISSING",
    )
    ...
    if len(candidate_records) != 1:
        raise DeliveryFailure(
            "V0253_TARGET_ACCEPTANCE_STATUS_CANDIDATE_CARDINALITY_INVALID"
        )
    if candidate_records[0].get("candidateBuildId") != candidate_build_id:
        raise DeliveryFailure(
            "V0253_TARGET_ACCEPTANCE_STATUS_CANDIDATE_IDENTITY_MISMATCH"
        )
    if (
        len(current) != 1
    ...
    delivery_note = safe_path(root, "docs/v0253-delivery.md", "V0253_CANDIDATE_NOTE_MISSING")
...
def audit_candidate_lineage(root: Path, manifest: Dict) -> None:
    ...
    if (
        ...
        raise DeliveryFailure("V0253_CANDIDATE_STATUS_INVALID")

    evidence = manifest.get("candidateEvidence", {})
    if evidence.get("automatedResult") != "candidate":
        raise DeliveryFailure("V0253_AUTOMATED_RESULT_MUST_REMAIN_CANDIDATE")
    ...
    if not re.fullmatch(r"[0-9]{8}", release_date):
        raise DeliveryFailure("V0253_RELEASE_DATE_INVALID")
    try:
    ...
    if (
        ...
        raise DeliveryFailure("V0253_CANDIDATE_BUILD_ID_INVALID")

    superseded = evidence.get("supersedes", {})
    current_archive_name = (
        "ai-wps-phase1-delivery-{0}-{1}-v0253.tar.gz".format(
    ...
    if (
        ...
        raise DeliveryFailure("V0253_SUPERSEDED_CANDIDATE_INVALID")
    _archive_source_binding(
        str(superseded.get("archiveName", "")),
        superseded.get("sourceCommit", ""),
        superseded.get("candidateBuildId", ""),
        "V0253_SUPERSEDED_CANDIDATE_SOURCE_BINDING_INVALID",
    )

    records = [item for item in status["records"] if isinstance(item, dict)]
    candidate_records = [item for item in records if item.get("status") == "candidate"]
    if len(candidate_records) != 1:
        raise DeliveryFailure("V0253_CANDIDATE_STATUS_CANDIDATE_CARDINALITY_INVALID")
    if candidate_records[0].get("candidateBuildId") != current_build_id:
        raise DeliveryFailure("V0253_CANDIDATE_STATUS_CANDIDATE_IDENTITY_MISMATCH")
    rejected = [
        item
        for item in records
    ...
    if previous_series == "1":
        ...
        if len(rejected) != 1 or rejected[0].get("status") != "rejected":
            raise DeliveryFailure("V0253_REJECTED_CANDIDATE_RECORD_MISSING")
        if rejected[0].get("candidateBuildId") != superseded.get("candidateBuildId"):
            raise DeliveryFailure("V0253_REJECTED_CANDIDATE_BUILD_ID_MISMATCH")
        if rejected[0].get("archiveSha256") != superseded.get("archiveSha256"):
            raise DeliveryFailure("V0253_REJECTED_CANDIDATE_DIGEST_MISMATCH")
        ...
        if previous_archive_source is not None and (
            ...
            raise DeliveryFailure("V0253_REJECTED_CANDIDATE_SOURCE_COMMIT_MISMATCH")
    ...
    if len(current) != 1 or current[0].get("status") != "candidate":
        raise DeliveryFailure("V0253_CURRENT_CANDIDATE_RECORD_INVALID")
    if current[0].get("archiveName") != current_archive_name:
        raise DeliveryFailure("V0253_CURRENT_CANDIDATE_ARCHIVE_NAME_INVALID")
    if current[0].get("sourceCommit") != source_commit:
        raise DeliveryFailure("V0253_CURRENT_CANDIDATE_SOURCE_COMMIT_INVALID")
    if current[0].get("archiveChecksumFile") != evidence.get("archiveChecksumFile"):
        raise DeliveryFailure("V0253_CURRENT_CANDIDATE_CHECKSUM_REFERENCE_INVALID")
...
def audit(root: Path) -> None:
    ...
    if manifest.get("version") != VERSION or manifest.get("adapter", {}).get("version") != VERSION:
        raise DeliveryFailure("V0253_VERSION_MISMATCH")
    if allowlist.get("version") != VERSION:
        raise DeliveryFailure("V0253_ALLOWLIST_VERSION_MISMATCH")
    baseline = manifest.get("baseline", {})
    if baseline.get("acceptedVersion") != BASELINE_VERSION or baseline.get("sourceStatus") != "candidate":
        raise DeliveryFailure("V0253_CANDIDATE_BASELINE_REQUIRED")
    evidence = manifest.get("candidateEvidence", {})
    if not COMMIT_RE.fullmatch(str(evidence.get("sourceCommit", ""))):
        raise DeliveryFailure("V0253_SOURCE_COMMIT_MISSING")
    release_date = str(manifest.get("releaseDate", ""))
    expected_checksum_name = "ai-wps-phase1-delivery-{0}-{1}-v0253.tar.gz.sha256".format(
        release_date, str(evidence.get("sourceCommit", ""))[:7]
    )
    if evidence.get("archiveChecksumFile") != expected_checksum_name:
        raise DeliveryFailure("V0253_ARCHIVE_CHECKSUM_EVIDENCE_MISSING")
    ...
    if (
        ...
        or policy.get("candidateAuditScript") != "scripts/audit_v0253_delivery.py"
    ):
        raise DeliveryFailure("V0253_CANDIDATE_POLICY_INVALID")
    ...
    print("v0253_delivery_audit=passed status=candidate version={0}".format(VERSION))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        if (args.archive is None) != (args.checksum_file is None):
            raise DeliveryFailure("V0253_ARCHIVE_CHECKSUM_ARGUMENTS_INCOMPLETE")
        ...
        print("v0253_delivery_audit=failed {0}".format(exc))
        return 1
    return 0
