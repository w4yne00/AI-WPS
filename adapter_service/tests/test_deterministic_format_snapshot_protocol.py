...
class DeterministicFormatSnapshotProtocolTests(unittest.TestCase):
    ...
    def test_completed_report_has_stable_instances_pagination_groups_and_exports(self):
        ...
        filtered_verified = self.service.list_issues(job["jobId"], data_status="verified")
        self.assertEqual(filtered_verified["total"], 2)
        filtered_restricted = self.service.list_issues(job["jobId"], data_status="restricted")
        self.assertEqual(filtered_restricted["total"], 0)
        filtered_not_assessable = self.service.list_issues(job["jobId"], data_status="not_assessable")
        self.assertEqual(filtered_not_assessable["total"], 0)
        with self.assertRaises(AdapterError):
            self.service.list_issues(job["jobId"], data_status="invalid_status")
