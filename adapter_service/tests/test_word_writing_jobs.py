...
class WritingJobStoreTests(unittest.TestCase):
    ...
    def test_smart_imitation_streaming_allows_cancel_while_blocking_does_not(self):
        blocking_worker = BlockingWritingWorker()
        coordinator = LongTaskCoordinator(max_running=1, max_queued=2)
        blocking_store = SmartImitationJobStore(worker=blocking_worker, coordinator=coordinator)

        req_blocking = make_request("client-im-block-cancel-1")
        blocking_store.start(req_blocking, "trace-im-block-1")
        self.assertTrue(blocking_worker.started.wait(timeout=2))

        # While running, blocking job should reject cancellation with LONG_TASK_NOT_CANCELLABLE
        with self.assertRaises(AdapterError) as ctx:
            blocking_store.cancel("client-im-block-cancel-1")
        self.assertEqual(ctx.exception.code, "LONG_TASK_NOT_CANCELLABLE")
        self.assertEqual(ctx.exception.status_code, 409)
        blocking_worker.release.set()
        self.wait_completed(blocking_store, "client-im-block-cancel-1")

        # History should contain the completed blocking imitation job
        history_entries = self.history_store.list_history("word.smart_imitation")
        self.assertEqual(len(history_entries), 1)
        self.assertEqual(history_entries[0]["jobId"], "client-im-block-cancel-1")
        ...
        class StreamingWorker:
            ...
            def imitate(self, request, trace_id, progress_callback=None, **kwargs):
                progress_callback("streaming")
                if hasattr(progress_callback, "publish_text"):
                    progress_callback.publish_text("仿写流式首段文本。")
                    progress_callback.flush()
                self.started.set()
                for _ in range(50):
                    if hasattr(progress_callback, "cancel_requested") and progress_callback.cancel_requested():
                        ...
                        raise LongTaskCancelled(partial_result={
                            "plainText": "仿写流式首段文本。",
                            "rewrittenText": "仿写流式首段文本。",
                            "partial": True,
                            "stopReason": "cancelled",
                        })
                    time.sleep(0.02)
                return {"rewrittenText": "最终仿写文本", "plainText": "最终仿写文本"}

        streaming_worker = StreamingWorker()
        streaming_store = SmartImitationJobStore(worker=streaming_worker, coordinator=coordinator)
        req_streaming = make_request("client-im-stream-cancel-1")

        with patch.dict("os.environ", {"AI_WPS_ENABLE_DIRECT_STREAMING": "1"}):
            streaming_store.start(req_streaming, "trace-im-stream-cancel-1")
            self.assertTrue(streaming_worker.started.wait(timeout=2))

            # Calling cancel while running MUST be accepted for streaming job
            cancel_resp = streaming_store.cancel("client-im-stream-cancel-1")
            self.assertEqual(cancel_resp["status"], "running")
            self.assertEqual(cancel_resp["phase"], "stopping")
            ...
            terminal_job = None
            for _ in range(50):
                terminal_job = streaming_store.get("client-im-stream-cancel-1")
                if terminal_job and terminal_job["status"] == "cancelled":
                    break
                time.sleep(0.02)
            ...
            self.assertIsNotNone(terminal_job.get("result"))
            self.assertTrue(terminal_job["result"].get("partial"))
            self.assertEqual(terminal_job["result"].get("plainText"), "仿写流式首段文本。")

            # Check history: cancelled imitation job MUST NOT be written to task history
            history_entries_after = self.history_store.list_history("word.smart_imitation")
            self.assertEqual(len(history_entries_after), 1)  # only the earlier blocking completed one
            self.assertEqual(history_entries_after[0]["jobId"], "client-im-block-cancel-1")

    def test_smart_write_and_smart_imitation_document_session_isolation(self):
        worker = BlockingWritingWorker()
        coordinator = LongTaskCoordinator(max_running=2, max_queued=2)
        write_store = SmartWriteJobStore(worker=worker, coordinator=coordinator)
        imitation_store = SmartImitationJobStore(worker=worker, coordinator=coordinator)

        doc_session = "shared-doc-session-1"
        write_req = make_request("client-write-shared-1", document_session_id=doc_session)
        imitation_req = make_request("client-imitation-shared-1", document_session_id=doc_session)

        # Both jobs can be submitted and run concurrently on the same document session
        write_job = write_store.start(write_req, "trace-write-1")
        imitation_job = imitation_store.start(imitation_req, "trace-imitation-1")

        self.assertEqual(write_job["status"], "running")
        self.assertEqual(imitation_job["status"], "running")

        # But a duplicate job of the same task type in the same document session is rejected
        write_req_dup = make_request("client-write-shared-2", document_session_id=doc_session)
        with self.assertRaises(AdapterError) as ctx:
            write_store.start(write_req_dup, "trace-write-dup")
        self.assertEqual(ctx.exception.code, "WORD_WRITING_DOCUMENT_TASK_BUSY")

        worker.release.set()
        self.wait_completed(write_store, "client-write-shared-1")
        self.wait_completed(imitation_store, "client-imitation-shared-1")
