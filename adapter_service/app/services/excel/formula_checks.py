...
LOCALLY_RECOGNIZED_FUNCTIONS = {
    ...
    "WEEKDAY",
    "YEAR",
}.union(NETWORK_FUNCTIONS, VERSION_SENSITIVE_FUNCTIONS)
...
def inspect_formula(formula: str, selection_address: str = "") -> Dict:
    ...
    unrecognized_functions = sorted(set(functions).difference(LOCALLY_RECOGNIZED_FUNCTIONS))
    if unrecognized_functions:
        risks.append(
            _risk(
                "FUNCTION_SUPPORT_REVIEW_REQUIRED",
                "公式包含未列入本地支持清单的函数，需在目标 WPS 核对可用性；本地清单不能判定目标版本是否支持。",
                ", ".join(unrecognized_functions),
            )
        )
