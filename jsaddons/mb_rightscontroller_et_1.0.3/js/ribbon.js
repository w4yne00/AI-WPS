...
function ReLinstenEvent(){
    try {
        ...
        // 打印前
        wps.ApiEvent.AddApiEventListener("WorkbookBeforePrint", OnDocumentBeforePrint);
    } catch (error) {
...
function DisableEdit(){
    try {
        var editShow = true;
        var printShow = true;
        if(g_app.ActiveWorkbook){
            // 编辑权限
            if (!IsFileHasRights(g_app.ActiveWorkbook.FullName, FILE_RIGHT.EDIT) || IsFileOA(g_app.ActiveWorkbook.FullName)) {
                editShow = false;
            }

            // 打印权限
            if (!IsFileHasRights(g_app.ActiveWorkbook.FullName, FILE_RIGHT.PRINT) && !IsFileHasRights(g_app.ActiveWorkbook.FullName, FILE_RIGHT.WMPRINT)) {
                printShow = false;
            }
        }
...
function OnDocumentBeforeClose(doc){
    try {
        ...
        if(g_hasRightsInfo){
            ...
            if(!doc.Saved){
                if(!IsFileHasRights(doc.FullName,FILE_RIGHT.EDIT)){
                    var bSelSave = wps.confirm("没有编辑权限，继续将丢失所做修改，是否继续？");
                    if(!bSelSave){
                        log("用户选择取消，不关闭文档");
                        wps.ApiEvent.Cancel = true;
                    }else{
...
function AddFileEvents(){
    try {
        ...
        // 打开
        wps.ApiEvent.AddApiEventListener("WorkbookOpen", OnDocumentOpen);
        // 保存前后
        wps.ApiEvent.AddApiEventListener("WorkbookBeforeSave", OnDocumentBeforeSave);
        wps.ApiEvent.AddApiEventListener("WorkbookAfterSave", OnDocumentAfterSave);
        // 关闭后
        wps.ApiEvent.AddApiEventListener("WorkbookBeforeClose", OnDocumentBeforeClose);
    
        // 如果存在wps.ApiEvent.RightsInfo和g_app.ActiveWorkbook.InvalidateRightsInfo
        // 需要增加监听，文档切换，权限控制事件
        // 如果不存在，则需要增加监听，复制前，粘贴前，打印前，切换[主要用于控制不存在权限控制接口时禁用另存菜单]
        //log(g_app.ActiveWorkbook.InvalidateRightsInfo);
        //log(wps.ApiEvent.RightsInfo);
        if(wps.ApiEvent.RightsInfo !== undefined && typeof g_app.ActiveWorkbook.InvalidateRightsInfo == "function"){
...
function OnAddinLoad(ribbonUI){
    ...
    AddFileEvents();
    // 响应第一个文件事件
    OnDocumentOpen(g_app.ActiveWorkbook);
