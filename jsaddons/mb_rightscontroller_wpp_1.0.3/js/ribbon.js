...
function ReLinstenEvent(){
    try {
        ...
        // 打印前
        wps.ApiEvent.AddApiEventListener("PresentationBeforePrint", OnDocumentBeforePrint);
    } catch (error) {
...
function DisableEdit() {
    try {
        var editShow = true;
        var printShow = true;
        if (g_app.ActivePresentation) {
            // 编辑权限
            if (!IsFileHasRights(g_app.ActivePresentation.FullName, FILE_RIGHT.EDIT) || IsFileOA(g_app.ActivePresentation.FullName)) {
                editShow = false;
            }

            // 打印权限
            if (!IsFileHasRights(g_app.ActivePresentation.FullName, FILE_RIGHT.PRINT) && !IsFileHasRights(g_app.ActivePresentation.FullName, FILE_RIGHT.WMPRINT)) {
                printShow = false;
            }
        }
...
function AddFileEvents() {
    try {
        ...
        // 打开
        wps.ApiEvent.AddApiEventListener("PresentationOpen", OnDocumentOpen);
        // 保存前后
        wps.ApiEvent.AddApiEventListener("PresentationBeforeSave", OnDocumentBeforeSave);
        wps.ApiEvent.AddApiEventListener("PresentationSave", OnDocumentAfterSave);
        // 关闭后
        // PresentationClose--关闭时
        wps.ApiEvent.AddApiEventListener("PresentationCloseFinal", OnDocumentAfterClose);

        // 如果存在wps.ApiEvent.RightsInfo和g_app.ActivePresentation.InvalidateRightsInfo
        // 需要增加监听，文档切换，权限控制事件
        // 如果不存在，则需要增加监听，复制前，粘贴前，打印前，切换[主要用于控制不存在权限控制接口时禁用另存菜单]
        //log(g_app.ActivePresentation.InvalidateRightsInfo);
        //log(wps.ApiEvent.RightsInfo);
        if (wps.ApiEvent.RightsInfo !== undefined && typeof g_app.ActivePresentation.InvalidateRightsInfo == "function") {
...
function OnAddinLoad(ribbonUI) {
    ...
    AddFileEvents();
    // 响应第一个文件事件
    OnDocumentOpen(g_app.ActivePresentation);
