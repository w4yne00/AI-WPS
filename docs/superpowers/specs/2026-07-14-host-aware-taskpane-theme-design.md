...
5. 三个宿主的主生成按钮仅显示高对比度纯文字，不显示矢量、图片或伪元素图标。
6. 不修改业务任务、adapter 接口、模型调用、轮询、复制、预览或回写逻辑。
...
| 宿主 | 识别色 | 主按钮底色 | 悬停/按下色 | 浅色背景 | 语义 |
| --- | --- | --- | --- | --- | --- |
| Word | `#2F6DB3` | `#2F6DB3` | `#265C98` | `#EAF2FB` | WPS 文字蓝 |
| Excel | `#237A4B` | `#237A4B` | `#1B643D` | `#EAF6EF` | WPS 表格绿 |
| PPT | `#D36B2C` | `#B95720` | `#99461A` | `#FFF1E7` | WPS 演示橙 |
...
Word、Excel、PPT 的 `#btn-run-primary` 统一采用纯文字主按钮：

- 保留现有按钮中文命令，例如“生成内容”“生成分析报告”“生成总结建议”；
- 文字使用白色，并与按钮底色保持至少 `4.5:1` 的对比度；
- Word 和 Excel 按钮使用宿主识别色，PPT 因亮橙与白字对比度不足而使用深橙 `#B95720`，悬停和按下时使用对应宿主深色；
- 删除 `#btn-run-primary::before` 及其背景图片、SVG、遮罩或占位宽度；
- 保持按钮高度、宽度、禁用态、加载文案和点击行为不变。

本项只约束主生成按钮。设置、返回、复制等工具按钮继续使用现有图标或文字形式。
...
- 从功能页打开设置时只切换 `home/settings` 可见视图，不调用完整 `switchMode`，不得清空结果预览、文档审查处理状态、未完成任务或智能编写回写可用性；
- 只有通过 `?mode=settings` 直接打开设置且尚未初始化功能页时，返回按钮才调用现有 `switchMode("smartWrite")` 完成默认页初始化。
...
1. 在 `formal-plugin-kit/tests/layout-smoke.test.js` 中增加静态契约断言并确认失败：
   ...
   - Word 设置快捷切换函数不调用结果、审查或回写状态重置函数；
   - Excel 设置返回智能分析；
   - Word、Excel 设置按钮在设置页使用 `is-back` 状态。
   - 三个 CSS 文件均不再定义 `#btn-run-primary::before`，主按钮保留宿主色背景和高对比度文字。
2. 进行最小实现，使新增测试通过。
...
5. 在 420×900 视口检查 Word、Excel、PPT 的功能页和设置页：
   ...
   - 设置/返回图标状态正确；
   - 三个宿主色可区分，且正文可读性不下降。
   - Word 四个功能页进入设置再返回后，原结果 DOM 和回写按钮可用状态保持不变。

## 8. 风险与保护措施

- **Word 模式丢失风险**：设置页不直接覆盖返回目标，使用独立 `lastTaskMode` 保存最后一个非设置功能。
- **任务状态丢失风险**：从已初始化的 Word 功能页进入设置时只切换视图，返回时复用未销毁的功能页 DOM；不得执行预览、审查和回写状态重置。
...
9. 三个宿主的主生成按钮均为纯文字按钮，无矢量、图片或伪元素图标，白色文字与按钮底色的对比度不低于 `4.5:1`。
