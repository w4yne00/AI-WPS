...
以下 issue #4 条目是当前工作树中的未发布开发变更，不属于已经打包的 `v0.19.1-alpha`：
...
- issue #4 当前工作树验证：Python 全量单测 `423 tests OK (skipped=42)`；全部 9 个前端测试文件、Word 前端语法检查和 `git diff --check` 通过。跳过项来自当前 bundled Python 缺 FastAPI 及既有环境条件；未重新生成 `v0.19.1-alpha` 交付包，也未替代下述历史真机验收。
