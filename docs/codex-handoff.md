...
- 设置：复用同一 adapter 配置、统一 API URL、统一模型 API Key 和智能分析工作流配置档案。
...
- 设置：复用同一 adapter 配置、统一 API URL、统一模型 API Key 和智能总结工作流配置档案。
...
1. 使用 `20260714-v0180` 单一正式交付包覆盖安装，关闭并重新打开 WPS，确认设置页“前端版本”为 `0.18.0-alpha`，且原 API URL、统一 API Key 与 `run/provider_api_keys/` 中的工作流密钥未丢失。
2. 设置页配置统一 API URL，例如 `https://aibot.chinasatnet.com.cn/v1`。
3. 分别为“智能编写”“智能仿写”“文档审查”“格式审查”“智能分析”“智能总结”保存两个具名工作流档案，切换后确认下一次任务命中所选档案；当前页和文档总结必须共用同一个 `ppt.slide_assistant` 档案。
...
- PPT/WPS WPP 的主标题和普通正文读取已有上一版本目标机基础；`v0.18.0-alpha` 的可选副标题、Markdown/DOCX 上传、文档提取、整套建议、长任务恢复、三宿主统一视觉和覆盖安装仍需用新正式包完成目标机验收。
