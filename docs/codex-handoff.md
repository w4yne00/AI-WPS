...
## Issue #234：Word：补充事实、处理冲突并核对结果（2026-09-26）

- **用户补充事实与编写要求分离**：任务窗格提供独立 `material-user-facts` 文本域与 `material-instruction` 编写要求（结构、篇幅、语气），在前端状态与请求载荷中严格解耦；补充事实统一转换为 `sourceType: 'user'`、`fileName: '用户补充事实'` 标明出处，与参考资料严格区分。
- **事实冲突检测与用户选择（杜绝时间自动覆盖）**：基于导入资料与用户补充事实，通过 `detect_material_conflicts` 提取项目预算、交付/初验/终验时间、建设工期、负责主体等关键事实，当存在 2 个以上不同来源的事实差异时在接口 `/word/material-composer/conflicts` 和窗格中呈现冲突差异；由用户手动选择采纳依据并携带 `conflictResolutions` 发起生成，系统严格不按文件修改时间或时间戳自动覆盖。
- **关键事实核对与未核验警示**：草稿生成后执行 `verify_key_facts`，针对正文中的关键数值、金额、日期、责任和承诺，对照引用原文进行事实校验；无依据内容显式标为 `待核对`，在侧栏和专用区域呈现，并声明“已标出无原文依据内容；AI 核对不伪造出处，亦不宣称发现全部冲突，请逐项核对”，杜绝伪造出处或虚假全知宣称。
- **缺项标注与确认写入选区/光标**：缺项在侧栏列出并在正文显式显示“〔待补充：具体信息〕”；允许用户在弹出确认弹窗后使用含缺项的章节草稿安全写入选区或在光标处插入；若用户取消确认则安全中止写入并保留草稿以供手动复制。
- **受控基准与全量门禁**：
  - 新增 `WordMaterialFactConflictReviewBenchmarkTests` 受控基准用例，覆盖可控冲突展示、人工选项解决、用户事实引用、未核对数值标出及缺项保留；
  - 后端 Python 3.8 全量测试：`1560 passed, 55 skipped`；
  - 交付预览门禁 `test_v0260_preview1_delivery.py`：`40 passed in 31.90s`；
  - 正式插件契约全套：`335 passed, 0 failed, 2 skipped`；
  - 原型套件 `wps-addon`：Vitest `12 passed`，Vite 生产构建成功；
  - 麒麟 V10 ARM64 / Python 3.8 真实环境实测：`33 passed`（后端核心与基准）+ `37 passed`（插件 Node 测试）；
  - Python 3.8 兼容性扫描 187 个文件全部通过，`git diff --check` 0 格式警告。


- **5 份资料与 10 万字符安全门禁**：单会话最多导入 5 份 DOCX、累计最多 100,000 Unicode 可读字符。超限返回 `MATERIAL_COUNT_OVER_LIMIT`（400）或 `MATERIAL_TEXT_OVER_LIMIT`（413），不截断。导入检查、片段编号分配及目录写入由同一锁保护，避免并发请求越过限额或重复编号。
- **会话级资料目录与后续章节复用**：基于 `documentSessionId` 管理内存资料目录；片段编号全局递增，块和片段携带 `materialId`，章节与表格行按唯一资料编号定位，同名文件互不覆盖。窗格重开或切回文档会查询服务端目录。
...
- **模型预算与运行中取消**：长段落和长表格单元格按原文分片并保留偏移，不强塞超预算片段；请求发送前复核完整提示词预算。四阶段在窗格映射为中文提示。直连和工作流请求已建立连接后，取消可中断等待响应头或读取响应体；连接建立仍受原有网络超时约束，不保证上游立即停止计费。取消和失败不产生可写草稿。
...
- **任务窗格目录与出处**：折叠目录按文件展示章节层级，点击章节可填写目标标题；总字数与出处侧栏沿用现有展示。目录查询失败保留本地状态，恢复查询可重试。
- **受控基准边界**：5 份合成 DOCX 累计 92,523 可读字符，逐项核对 5 个预设事实、3 行表格及所选片段原文。计时分别标明模拟模型的任务耗时与纯提取耗时，不作“零幻觉”或真实模型质量结论。真实资料人工预标注、真实模型及真实 WPS 验收仍未完成。
- **2026-09-26 审查修复验证**：麒麟 V10 ARM64 / Python 3.8 后端 `1555 passed / 55 skipped`；本机正式插件含真实 Chrome 目录点击与窄窗恢复 `333 passed`；原型 `12 passed` 且构建成功；80 个生产文件 Python 3.8 兼容扫描、差异检查、临时组装目录的通用及 Preview 交付审计通过。此次合成基准实测建库 7.48 ms、第一章模拟模型任务 16.85 ms、第二章模拟模型任务 13.56 ms、第五章纯提取 3.80 ms；这些耗时不含真实模型往返。复核补充修复了 HTTP 错误正文等待期间取消与迟到目录响应覆盖新导入的问题。未生成正式交付归档。
...
- 选区与光标智能区分：当用户选中文本时，写入操作精确替换选区；当光标折叠（无选区）时，在光标处安全插入草稿，不破坏选区外正文。
- 目标章节动态保护：草稿生成完成后，若用户在窗格修改“章节标题”或在生成期间目标章节发生变化，写入按钮自动禁用并高亮警告“目标章节已变更，写入已暂停”，要求重新生成或恢复章节标题，防止将草稿写错位置。
- 文档会话隔离与状态门禁：严格绑定 `documentSessionId`，切换文档立即重置写入按钮与草稿状态；仅允许 `completed` 终态草稿写入，排队、生成中、已取消或失败草稿严格不可写回。
- 写入失败恢复路径：写入发生异常时保留草稿内容，明确提示失败原因并引导用户使用“复制草稿”手动粘贴，绝不误报成功或清空草稿。
- PR #243 审查修复：全选正文时拒绝写入；生成时锁定选区位置与内容，变化或重开窗格后只保留预览和复制，写入需重新生成；写入后回读正文，区分完整、未写入和可能部分写入，后者先核对并修正文档，不直接重复粘贴。
- 回归测试与麒麟验证：新增 5 项核心模块单元测试及 5 项窗格契约测试（含真实 Chrome 视口）；麒麟 V10 ARM64 / Python 3.8 全量后端 `1530 passed / 55 skipped`；正式插件在麒麟为 `303 passed / 1 skipped`（缺少 `agent-browser`），本机真实 Chrome 全量插件 `314/314 passed`；`wps-addon` `12/12 passed` 且生产构建成功；Python 3.8 兼容扫描 186 个文件与 `git diff --check` 通过。
...
- 公开接口：`POST /word/material-composer/jobs` 接收 `materialId`、`documentSessionId`、`clientJobId`、`sectionTitle`、`instruction`；`GET /word/material-composer/jobs/{jobId}?documentSessionId=...` 恢复查询；`POST /word/material-composer/jobs/{jobId}/cancel` 接收 `documentSessionId`。FastAPI 与 standalone 共用后台处理；同一请求编号复用原任务，不同输入或会话拒绝复用；两个 POST 入口均在解析前执行 64 KiB 请求体上限。
...
- PR #242 审查修复：不确定提交在重开和 404 后继续固定原请求；同一文档只接受最后一次资料导入响应；资料章节草稿模型保存提示使用正确任务名称。
- 审查修复验证：麒麟 V10 ARM64 / Python 3.8 后端 `1530 passed / 55 skipped`；正式插件在麒麟为 `302 passed / 2 skipped`，跳过项均因测试机未安装 `agent-browser`，同一全套在本机真实 Chrome 为 `304/304`；原型 `12/12` 且生产构建成功，麒麟 Python 3.8 兼容扫描 84 个文件及差异检查通过。
