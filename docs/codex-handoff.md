...
- 选中文本改写和续写。
- 智能改写/续写任务窗格会显性展示风格、侧重点、篇幅和输出约束对应的提示词片段，并将用户输入区按当前任务标记为“改写提示词”或“续写提示词”；其他任务窗格不展示提示词拆解卡片。
...
- 智能改写和智能续写会把完整提示词放入 Chat 顶层 `query`，并在 `inputs` 内同步 `text`、`source_text`、`mode`、`rewrite_mode`、`query`、`prompt`，适配 Dify Chat 直接对话和 User Input 变量编排两种方式。
...
`v0.10.3-alpha` 放弃“单 Dify 工作流 + 判断节点分流”作为主路线。adapter 负责按任务选择 Dify 应用或工作流：
...
- 对 `word.rewrite` 和 `word.continue`，`inputs` 内额外包含 `source_text`、`text`、`mode`、`rewrite_mode`、`query`、`prompt`、`user_instruction`、`rewrite_style`、`focus_point`、`length_mode`。
...
```text
继续基于 v0.10.3-alpha 在麒麟 V10 ARM 目标机验证多任务 Dify 路由。请重点验证：1）/config 是否返回每个 taskRoute 的 path、payloadStyle、apiKeyRef 和 configured 状态；2）设置页能否分别保存 rewrite、continue、proofread、format_preview、technical_review 的 API Key；3）智能改写/续写是否命中 chat 应用；4）格式校对/技术审查是否命中对应 workflow 并能解析 outputs.result；5）如果企业封装接口字段与 Dify 标准不同，请记录请求/响应样例，用于新增 payloadStyle 兼容层。
```
