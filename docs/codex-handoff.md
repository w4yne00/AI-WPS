...
最新提交：以 `git log -1 --oneline` 为准。本文件已随 `v0.8.0-alpha` 格式校对增强一并更新。
...
- 格式校对会随请求提交 `documentStructure`，包含段落、标题和基础样式结构。
...
- 综合语言审校已设计为 Dify User Input 结构化输入，覆盖错别字、语病、表述不规范、逻辑不清、章节命名不统一。
...
- 格式校对结果支持问题分类：`format`、`typo`、`grammar`、`expression`、`logic`、`heading_consistency`。
...
- `docs/phase1-proofread-next-version-backlog.md`：记录格式校对下一版本暂不实现但需要继续规划的能力。

### adapter 服务

- `adapter_service/app/main.py`：FastAPI 应用入口，注册健康、配置、模板、provider、Word API 路由，版本为 `v0.8.0-alpha`。
...
- `adapter_service/app/services/provider_client.py`：企业 AI HTTP provider 封装，负责构造 prompt、调用 `/chat-messages`、解析结果、mock 回退、API Key 文件读写、综合审校 payload 和 AI 审校结果解析。
...
- `adapter_service/standalone_adapter.py`：无 FastAPI/uvicorn 环境的轻量 HTTP 服务兜底，版本同步为 `v0.8.0-alpha`。
...
- `formal-plugin-kit/wps-ai-assistant_1.0.0/taskpane-helpers.js`：可测试的前端辅助函数，包含选区处理和 `documentStructure` 构造。
...
- 格式校对下一版本备忘见 `docs/phase1-proofread-next-version-backlog.md`，包括页眉页脚、页码、表格深度抽取、文档内高亮定位、自动批量修复、多模板 RAG 检索配置界面和当前章节范围。
...
- `content.documentStructure`：插件生成的结构化文档数据，用于 Dify User Input 节点。

`documentStructure` 主要字段：

- `doc_name`：文档名称。
- `template_id`：模板 ID。
- `selection_mode`：`document` 或 `selection`。
- `page_setup`：页面设置；当前为最佳努力抽取，拿不到时为空对象。
- `paragraphs`：段落数组，包含文本、样式名、字体、字号、粗体、斜体、下划线、对齐、标题层级、行距、缩进、段前段后。
- `headings`：标题数组，包含标题级别、标题文本、段落索引。
- `tables`：预留，当前为空数组。
- `captions`：预留，当前为空数组。
- `capabilities`：说明当前抽取能力是否成功，例如 `paragraph_style_extracted`、`table_extracted`。
...
格式校对请求会把 `documentStructure` 传入 adapter，adapter 进一步组织为企业 Dify User Input：
