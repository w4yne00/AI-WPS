...
当前阶段是一期开闭环收口：平台底座 + Word 能力 + 离线交付体系。当前版本重点收紧 adapter 多工作流路由和每任务密钥选择，消除全局密钥状态对任务路由排查的干扰，并为目标机联调提供路由诊断信息。
...
- `/provider/route-diagnostics` 返回脱敏后的任务路由诊断信息。
- `/templates` 模板列表。
...
- 命名任务密钥只读取 `adapter_service/run/provider_api_keys/<apiKeyRef>`，不再回退全局环境变量或旧默认密钥文件。
- 旧目标机 `config/adapter.json` 缺少新版默认 `taskRoutes` 时，adapter 会从 `adapter.example.json` 自动补齐缺失任务路由。
...
docs/superpowers/plans/2026-05-19-task-route-key-diagnostics-plan.md
```
...
tar -tzf dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260519.tar.gz | rg 'wps-ai-assistant_1.0.0/(manifest.json|taskpane.js|taskpane.html)|config/adapter.example.json|dify-task-routes-path-apikeyref|install_phase1.sh|publish.xml|start_uvicorn_adapter.sh'
tar -tvzf dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260519.tar.gz | rg 'installer/install_phase1.sh|scripts/phase1_smoke_test.sh|packages/adapter-start-kit/scripts/(enable_exec_permissions|check_health|start_uvicorn_adapter)\.sh'
tar -xOf dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260519.tar.gz ai-wps-phase1-delivery-20260519/packages/adapter-start-kit/scripts/start_uvicorn_adapter.sh | rg 'EXPECTED_VERSION|0\.11\.1-alpha'
tar -xOf dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260519.tar.gz ai-wps-phase1-delivery-20260519/packages/wps-ai-assistant_1.0.0/taskpane.html | rg 'provider-auth-line|密钥：未检测|任务接口'
```

结果：

- Python 单测：`48 tests OK (skipped=9)`。
...
- 交付包校验：包含 `manifest.json`、`taskpane.js`、`taskpane.html`、`adapter.example.json`、`publish.xml`、Dify 路由手册和 `start_uvicorn_adapter.sh`。
- 交付包权限校验：安装脚本、联调脚本和 adapter 启动/健康检查脚本权限为 `rwxr-xr-x`。
- 交付包内容校验：`start_uvicorn_adapter.sh` 内 `EXPECTED_VERSION=0.11.1-alpha`；任务窗格不包含 `provider-auth-line` 或 `密钥：未检测`，保留 `任务接口`。

## 10. 下一轮任务建议 Prompt

```text
继续基于 v0.11.1-alpha 在麒麟 V10 ARM 目标机验证智能编写 Workflow 闭环。请重点验证：1）设置页顶部不再显示全局“密钥未配置”状态；2）任务接口区域只显示每任务 API Key 配置；3）旧 adapter.json 缺少 taskRoutes 时 /config 仍返回 word.smart_write；4）/provider/route-diagnostics 中 word.smart_write 的 url、apiKeyRef、payloadStyle、outputKey、configured、authSource 是否正确；5）保存 smart_write 任务 Key 后 /config 是否返回 word.smart_write configured=true；6）智能编写是否调用 /word/smart-write；7）Dify Workflow Start 是否接收到 source_text、write_action、style、focus、length、user_prompt、selection_mode、trace_id；8）Output result 是否能在任务窗格结果预览显示，并可应用到选中文本。
```
