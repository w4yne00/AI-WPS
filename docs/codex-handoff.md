...
- 模板下拉会合并后端 `/templates` 返回结果和本地兜底模板，避免旧 adapter 或异常返回导致只显示通用模板。
...
- `docs/operations/phase1-v0.9.1-deployment.md`
...
- `formal-plugin-kit/wps-ai-assistant_1.0.0/taskpane.html`：定义任务窗格结构，技术审查文档类型收敛为三项。
- `adapter-start-kit/scripts/start_uvicorn_adapter.sh`：优先启动 uvicorn，并在发现旧版本 adapter 占用端口时替换旧进程。
- `adapter_service/tests/test_packaging_scripts.py`：覆盖启动脚本旧进程替换逻辑、模板兜底合并和技术审查三类文档配置。
...
- `docs/operations/phase1-v0.9.1-deployment.md`：新版本目标机部署手册。
...
- 目标机如果已经启动旧版 adapter，必须使用 `v0.9.1-alpha` 的 `start_uvicorn_adapter.sh` 重新启动；新版脚本会自动识别并替换旧进程。
...
- `adapter_service.tests.test_enterprise_provider`：`OK (skipped=1)`。
- `adapter_service.tests.test_packaging_scripts`：`OK`。
- `compileall`：通过。

跳过原因：本机缺少 `pydantic`，`test_document_structure_is_accepted_in_word_request` 按测试条件自动跳过。

未执行完整 FastAPI 集成测试，原因是本地 Python 环境缺少 `fastapi`、`pydantic` 等目标运行依赖；目标机安装离线依赖后可补跑 `/health`、`/templates`、`/word/technical-review` 联调。

## 10. 下一轮任务建议 Prompt

```text
继续基于 v0.9.1-alpha 在麒麟 V10 ARM 目标机验证新交付包。请重点验证：1）start_uvicorn_adapter.sh 是否能替换旧版 adapter 并返回 version=0.9.1-alpha；2）格式校对和智能排版模板下拉是否同时出现“技术文件格式及书写要求”和“通用办公模板”；3）技术文档审查三类文档类型是否能切换对应默认提示词；4）/word/technical-review 是否不再 404，并能通过 Dify task_id=word.technical_review 路由返回结构化 JSON。
```
