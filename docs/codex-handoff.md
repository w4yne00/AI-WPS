...
版本规则号：`AI-WPS-P1-WORD-0.12.7-20260528`
...
2026-05-25 针对现场智能编写仍显示纯文本的问题核查后确认：仓库和上一交付包已包含 Markdown 渲染器，但 WPS 插件目录和任务窗格地址固定，存在加载旧静态资源缓存的风险。本版本对任务窗格页面以及 CSS/JS 引用增加 `0.12.1-alpha` 构建参数，在诊断区显示前端版本，并在 `/provider/debug-last` 中增加不泄露正文的 Markdown 特征摘要。

2026-05-27 补充智能编写 Dify 工作流操作手册，固定大模型节点 `SYSTEM` 提示词、`{{sys.query}}` 使用方式、Markdown 输出约束和 `/provider/debug-last.response.answerFormat` 联调判断方法；一期交付包同步包含该手册。

2026-05-27 修复智能排版长文档预览覆盖不完整问题：此前 Dify 角色识别提示词仅包含前 120 个非空段落，现改为每 120 段一批覆盖整篇文档并合并合法角色结果；任务窗格新增全文扫描、AI 识别和本地兜底统计，明确列表只展示需调整的格式项而非正文片段。

2026-05-27 前台视觉更新为 `A 雾蓝银白` 方案：任务窗格的背景、按钮、焦点态和 Markdown 层次色统一调整为明亮蓝灰白风格，六个 Ribbon PNG 图标原位换色并保持映射及尺寸不变；本项不涉及功能流程、adapter 或接口配置变更。

2026-05-28 智能编写任务窗口按 `方案 A` 做紧凑化：四个编写设置压缩为更短的控制区，隐藏常驻提示词明细卡，保留“当前要求”摘要并扩大 Markdown 结果预览高度。表达风格、侧重点和篇幅菜单改为更贴近国企技术方案/汇报材料的选项，adapter 侧保留旧值别名兼容。

2026-05-28 继续修复智能排版 Dify 输出不符合预期的排错问题：智能排版当前逻辑要求 Dify 只返回段落角色 JSON，adapter 负责按模板写回格式。现已兼容回复节点把 JSON 包在 `result`、`data`、`outputs`、`answer` 等字段内或直接返回 JSON 数组的情况；如果 Dify 返回原文、Markdown 正文或解释性文字，预览 summary 会暴露 `aiFallbackReason`，任务窗格会显示“AI 识别提示”，避免继续被误判为只读取了部分文档。

2026-05-28 目标机智能排版预览显示“全文扫描段落：0、识别来源：local”，且 Dify 后台无调用记录。根因是 WPS 段落对象在现场表现为 COM 集合 `Paragraphs.Count` + `Paragraphs.Item(index)`，前端此前只按数组 `length` 遍历，导致没有向 adapter 提交任何段落，adapter 因段落数为 0 不会调用 Dify。本版本将段落采集和格式写回统一适配 WPS 集合，同时智能排版固定按全文预览，避免选中文本状态误导排版链路。
...
- `/word/format-preview` 现在返回 `targetProperties`、`role`、`confidence`、`summary.provider` 和全文覆盖统计；WPS 侧按预览计划应用页面设置、字体、字号、段落对齐、行距、缩进、段前段后和大纲级别。
...
- 智能排版对全文非空段落按每批最多 120 段调用 Dify 角色识别，合并有效结果，未识别或单批失败的段落继续使用本地规则；预览区显示覆盖统计但仅列需要调整的格式项。
- 智能排版 Dify 角色结果解析兼容顶层 `paragraphs`、常见包装字段和 JSON 数组；当 AI 请求成功但角色 JSON 不可解析时，`summary.aiFallbackReason` 会返回具体原因，任务窗格同步显示 AI 识别提示。
- 任务窗格段落采集支持 WPS COM 风格的 `Paragraphs.Count` / `Paragraphs.Item(index)` 集合，并继续补充 `Paragraph.Range.Text`、`Content.Paragraphs`、`Range().Paragraphs` 和全文文本拆段兜底；智能排版应用预览也通过同一集合适配按段落索引写回。
- 全文文本拆段兜底只保证预览和 Dify 联调不中断；如目标 WPS 没有暴露可写回的段落集合，点击“应用预览”会提示未写回正文，避免误报已应用。
- 任务窗格发送 `/word/smart-write`、`/word/format-preview` 前会清洗 WPS 宿主对象属性，避免函数、COM 对象或 WPS 枚举值进入 JSON 后触发 HTTP 422；后端 `WordDocumentRequest` 也对缺失 `documentId/plainText`、对象形态字号/样式、非标准 selectionMode 和下划线枚举做容错。
- FastAPI 请求校验失败时新增统一处理：返回 adapter 标准错误体，并刷新 `/provider/debug-last` 为 `provider=adapter`、`skipReason=request_validation_failed`，附带脱敏字段路径和错误类型。
- 智能排版在未读取到正文段落、任务密钥未配置、Dify 请求失败或返回不可解析角色时，`summary.aiFallbackReason` 和 `/provider/debug-last.skipReason` 会给出明确原因；未真实转发 Dify 时也会刷新 debug-last，避免看到旧任务的诊断。
- 设置诊断区显示实际加载的前端版本；`/provider/debug-last.response.answerFormat` 显示标题、编号列表、无序列表、加粗和段落空行等 Markdown 特征布尔值。
- 任务窗格和 Ribbon 图标采用雾蓝银白配色，图标路径、控件映射、任务调用与后端 API 均保持不变。
- Ribbon 打开任务窗格时追加 `build=0.12.7-alpha`，任务窗格 CSS/JS 引用追加同一版本参数；本轮缓存参数用于确保目标机加载新的智能编写紧凑布局、菜单文案、智能排版诊断提示和 WPS 段落集合适配。
- 智能编写前端默认选项改为 `standard`、`complete`、`same`；后端 `provider_client` 对 `default`、`formal`、`conclusion`、`risk`、`next_step`、`implementation` 等旧值仍做提示词别名兼容。

## 3. 需要重点保护的既有逻辑

- WPS Ribbon 五个入口：智能编写、格式校对、智能排版、技术文档审查、设置。
- 智能编写 UI 中的动作、风格、侧重点、篇幅、用户补充要求，以及“不允许原样返回”的提示词约束。
- 智能编写新菜单值和旧值兼容映射：前端只展示新选项，adapter 必须继续识别旧 payload 值。
...
- `adapter-start-kit/scripts/start_uvicorn_adapter.sh`：`EXPECTED_VERSION=0.12.7-alpha`。
...
docs/superpowers/specs/2026-05-25-smart-write-markdown-runtime-verification-design.md
docs/superpowers/plans/2026-05-25-smart-write-markdown-runtime-verification-plan.md
docs/superpowers/specs/2026-05-27-smart-format-full-document-preview-design.md
docs/superpowers/plans/2026-05-27-smart-format-full-document-preview-plan.md
docs/superpowers/specs/2026-05-27-blue-gray-taskpane-ribbon-theme-design.md
docs/superpowers/plans/2026-05-27-blue-gray-taskpane-ribbon-theme-plan.md
docs/superpowers/specs/2026-05-28-smart-write-compact-soetech-options-design.md
docs/superpowers/plans/2026-05-28-smart-write-compact-soetech-options-plan.md
```
...
- Python 单测：`69 tests OK (skipped=8)`；新增回归覆盖智能编写国企技术方案菜单提示词、旧选项别名兼容、智能排版包装 JSON 解析、全文拆段兜底、无段落不调用 Dify 的诊断、AI 解析失败诊断，以及 WPS 宿主对象形态入参不会触发 HTTP 422，并保留既有格式校对和技术审查回归。
- JS 布局冒烟：通过；覆盖新菜单、新默认值、紧凑布局、隐藏提示词明细卡、智能排版全文预览入口和 `0.12.7-alpha` 缓存参数。
- JS helper 测试：Markdown 段落/单换行、表格、分隔线、基础格式、危险 HTML/链接转义、WPS `Paragraphs.Count`/`Item()`、`Paragraph.Range.Text`、`Content.Paragraphs` 和全文文本拆段读取通过。
- `taskpane.js` 语法检查：通过。
- 任务窗格/Ribbon 视觉：雾蓝银白配色 smoke 断言通过；本轮用 430px 窄侧栏打开智能编写静态页，确认设置区为两列、提示词明细卡隐藏、结果预览 `min-height=360px`。
- 本轮使用 bundled Python 运行回归，环境包含 Pydantic 但缺少 FastAPI，因此 FastAPI TestClient 相关 8 项按测试文件 skip；不依赖 FastAPI 的 provider、打包脚本、智能排版模型解析、前端和编译检查均已通过。

## 8. 下一轮目标机验证建议

继续基于 `v0.12.7-alpha` 在目标机验证智能编写 Markdown 与智能排版闭环：
...
5. 长文档智能排版预览顶部应显示“全文扫描段落”“AI 识别段落”“本地兜底段落”，且“待调整项”只是需要写回格式的段落数量，不代表系统只读取了局部正文。
6. `/provider/debug-last` 在真实转发后应显示 `taskType=word.smart_format`，`bodyKeys` 包含 `inputs`、`query`、`response_mode`，`inputsKeys=["query"]`；长文档按批发送时该接口只反映最近一次 Dify 请求。
7. 如果智能排版预览显示“AI 识别提示：Dify 未返回段落角色 JSON”，说明 adapter 已尝试调用 Dify，但回复节点输出不是段落角色 JSON；需要在 Dify 预览页检查回复节点原始输出。
8. 如果智能排版仍显示“全文扫描段落：0”，优先确认设置页诊断区“前端版本”为 `0.12.7-alpha`；如果版本正确，查看 `/provider/debug-last.skipReason`，`no_paragraphs` 表示当前 WPS 没有暴露正文段落或全文 Range，需要继续采集目标机对象形态。
9. 执行智能编写后，在设置页诊断区确认“前端版本”为 `0.12.7-alpha`；`/provider/debug-last.response.answerFormat.containsMarkdown=true` 时，任务窗格应渲染返回内容中的标题、列表、加粗和分段，并显示新的“技术方案正式 / 保持信息完整 / 保持篇幅”默认菜单。
10. 确认任务窗格及 Ribbon 图标已显示为雾蓝银白样式；若仍显示旧绿色，请完全退出并重新启动 WPS 后再次导入本交付包插件资源。

## 9. 遗留项和下一步设计方向

- 智能排版暂缓修改：目标机已确认 `word.smart_format` 可按任务级 API Key 命中独立 Dify 工作流，但长文档调用时 Dify/模型容易受上下文窗口和最大输出 token 限制影响。当前实现每批最多 120 段、每段最多 300 字进入角色识别提示词，单批输入仍可能过大。后续如恢复智能排版，应改为本地规则优先、只把不确定段落送 AI、动态控制单批字数、降低每批段落数、增加批次超时和失败兜底。
- 格式校对下一版重新设计：避免复用智能排版的大上下文模式，改为“本地确定性格式检测 + 小批量 AI 语言质量审校 + 结果聚合去重”。能力范围应覆盖错别字检测、格式检测、语言逻辑表达检测和通畅性检测；Dify 只返回短 JSON 问题列表，不返回全文改写内容。
