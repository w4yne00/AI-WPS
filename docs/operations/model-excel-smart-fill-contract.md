...
当前来源先行预览不开放目标绑定和“写入内容”入口；写入属于后续目标映射票（#148），不是本生成合同的可操作行为。

产品写回规则仍是：模型没有地址控制权。目标映射完成后，任务窗格先展示逐项预览，用户可以编辑、取消勾选或单独重试；只有用户明确点击“写入内容”后，前端才重新读取并核对工作簿、工作表、目标地址、原值、公式/保护状态和来源快照。普通文本/数值覆盖需要二次确认，任一冲突或宿主写入失败都会停止并按逆序补偿已写入单元格；补偿失败时显示准确人工核对地址。不提供用户撤销按钮或原生 Undo。
