...
本合同同时适用于模型直连和工作流平台。Adapter 负责来源边界、逐行填写项和严格解析；模型只根据已授权的来源上下文，为不可猜测的 `itemId` 返回文本或普通数值。目标地址在生成请求中不存在，任务窗格只展示和复制结果，不绑定写入目标。
...
模型不得返回或执行公式。以 `=`, `+`, `-`, `@` 开头的业务文本仍按普通文本返回；任务窗格不会执行这些内容。日期和布尔值首版按文本处理。无法从授权上下文可靠得到的事实必须返回 `insufficient_information`，不得使用外部知识补齐姓名、机构、日期、金额、数量、地域或责任主体。
...
模型生成阶段仅接受来源区域，模型无目标地址感知权。任务完成后，任务窗格使用安全 Markdown 在结果预览卡片中按来源行展示模型输出。

- 前端不提供“写入内容”、结果编辑、勾选排除、逐项重试或目标区域绑定。
- “复制结果”输出两列制表符文本：第一列是来源行，第二列是模型输出；值中的制表符、回车和换行统一替换为空格，避免粘贴时拆出额外单元格。
- 复制操作不修改工作簿。用户必须人工核对结果，并自行选择对应单元格完成粘贴。
- Adapter 保留的旧写回兼容接口不属于当前任务窗格正式路径，前端不会调用。
