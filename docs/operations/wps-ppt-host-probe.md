...
本手册优先验证 Linux WPS 演示 JS 插件能否稳定读取：
...
宿主读取本身不修改幻灯片；“智能总结”模型联调需要连接 adapter，但结果仍只允许预览和复制。

发布前必须完成本手册中的宿主读取场景，并确认 `ppt.slide_assistant` 两种模式保持只读。
...
- `ribbon.xml`：只显示“智能总结”和“设置”。
- `taskpane.html`：当前页总结、文档总结和设置界面。
...
点击“智能总结”，选择“当前页总结”，再点击“生成本页总结”。当前页摘要条应显示页码、主标题、副标题、正文字数、相邻标题和截断状态；下列 JSON 是内部读取契约，用于核对字段，不要求直接显示在界面中。
