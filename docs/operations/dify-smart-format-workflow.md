...
`v0.12.2-alpha` 起，长文档不再只识别前 120 个非空段落。adapter 会按最多 120 段一批向同一个智能排版 Dify 应用发送角色识别请求，合并每批有效结果；某批返回失败或某段没有角色时，只对相应段落使用本地模板规则兜底。
...
预览窗口显示的是“整篇文档扫描后仍需要调整的格式项”，不是原文全文或排版后正文副本。顶部统计应区分：
...
`待调整项` 少于 `全文扫描段落` 属于正常情况，表示其余段落原本已符合模板规则，无需重复写回。
...
智能排版真实转发成功时，应看到类似信息。长文档可能产生多批请求，`/provider/debug-last` 展示的是最近一批请求的诊断信息：
...
4. `bash scripts/status_adapter.sh` 是否显示当前服务是 `uvicorn` 且版本不低于 `0.12.2-alpha`。
...
智能排版的预览对象是格式变更计划，不是模型改写后的正文。任务窗口会显示全文扫描覆盖数量，并只列出需要应用格式的段落；用户点击“应用预览”后，adapter 生成的模板格式属性才写回对应 Word 段落，正文内容保持不变。
