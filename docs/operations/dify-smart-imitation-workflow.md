...
- 写作规范：用户可选择自动匹配、G企技术材料、网络安全技术材料、党政公文或不使用写作规范；该选择与智能编写分别记忆。

写作规范冲突按以下顺序处理：

1. 保护项，包括事实、专有名词、数字、日期、责任主体、条款和规范性强度。
2. 用户本次明确要求。
3. 组织术语和组织规则。
4. 仿写模板的结构与句式意图。
5. 当前场景的预置文体与术语基线。
...
模板结构与句式意图高于预置文体，但不得改变保护项或组织术语。去模板化规则不得破坏用户明确要求保留的模板结构。adapter 仍只发起一次模型调用，不执行二次润色。
...
- 显示规范应用摘要以及非阻断的“需要核对”和“表达建议”；模板事实默认不要求保留，用户明确指定保留的模板日期、数字、责任主体等保护项除外；本地检查失败时保留模型正文。
- 不提供对照视图。
- 不提供应用预览。
- 不写回 Word 正文。
