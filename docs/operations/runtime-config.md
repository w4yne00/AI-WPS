...
- **当前发布裁决（Issue #213 / ADR-0132）**：
  - 自动化单元测试、确定性性能场景与 ≥30 次采样基准在本地与麒麟 V10 SP1 ARM64（Python 3.8.10）全量通过，耗时指标、分片抽取与取消槽位释放均满足 p95/p99 门禁。
  ...
  - **正式裁决**：`v0.26.0-preview.1` 及后续版本中严格保持 `AI_WPS_ENABLE_DIRECT_STREAMING=0`（默认关闭）。流式能力仅在显式配置环境变量 `=1` 时启用，坚决贯彻 Fail-Closed 安全与稳定性原则。
