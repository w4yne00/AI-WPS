...
| 交付状态 | 当前没有活动的自动化候选；`20260824-afe109c` 因目标机验收审计/测试在缺失必测第 8 或第 9 行时未 fail closed 而拒绝；Issue #59 仍为 `manual-pending` |
| 一期交付包 | 被拒绝归档 `ai-wps-phase1-delivery-20260824-afe109c-v0251.tar.gz`，SHA-256：`e3d4da0d1d8e1edc619d2101f45afb104ef8e3a6e5197e4b8e59b46513f78c6b`；归档字节冻结，等待重建 |

### v0.25.1-alpha 交付状态

当前没有活动的自动化候选。冻结的 `20260824-afe109c` 记录 candidate build ID 为
...
`e3d4da0d1d8e1edc619d2101f45afb104ef8e3a6e5197e4b8e59b46513f78c6b`，已登记为
`rejected`，原因是目标机验收审计/测试在缺失必测第 8 或第 9 行时未 fail closed；重建前不形成新的候选。目标 WPS GUI、真实模型和 Issue #59 人工验收仍为 `manual-pending`。
...
历史候选源码 `ccad09fb1d8019da3a40f14610ab3bd75de1ec23` 曾删除批次级伪范围、加入精确模型能力表、脱敏空正文诊断和不可重试边界，并将旧工作流档案改为版本化一次迁移；但其跨运行时哈希契约仍有阻断缺陷，归档已拒绝。当前没有自动化候选；冻结的 `afe109c` 归档已拒绝，目标 WPS、真实模型和 Issue #59 人工验收仍为 `manual-pending`。
...
| Python 3.8 生命周期门禁 | 冻结 `afe109c` 生命周期证据作为 rejected 证据保留；目标机验收仍独立待完成 |
| 归档校验 | 被拒绝 `afe109c` 归档 SHA-256 为 `e3d4da0d1d8e1edc619d2101f45afb104ef8e3a6e5197e4b8e59b46513f78c6b`；上一 `799adf9` 归档保持原字节并以 `rejected` 保留，SHA-256 为 `5f15e385358dcaea987e62f43cd2db1b943696372a7867449a986cdfc403f67c` |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 因目标机验收审计/测试在缺失必测第 8 或第 9 行时未 fail closed，拒绝冻结的 `20260824-afe109c`；重建前没有活动候选，Issue #59 仍为 `manual-pending` |
