...
本机 Adapter（默认 `127.0.0.1:18100`）承接九类任务：工作流平台按功能独立配置并走 `/chat-messages`；模型直连使用共享直连服务，只录入一次 URL 和 Key，任务选择模型和参数后走 OpenAI 兼容 `/chat/completions`。运行时不回退统一 URL 或统一 Key。生产环境默认关闭模拟结果。
