...
| 交付状态 | `20260824-799adf9` 是唯一自动化 `candidate`；Issue #59 目标验收仍为 `manual-pending` |
| 一期交付包 | 当前候选归档 `ai-wps-phase1-delivery-20260824-799adf9-v0251.tar.gz`，SHA-256：`5f15e385358dcaea987e62f43cd2db1b943696372a7867449a986cdfc403f67c` |

### v0.25.1-alpha 交付状态

当前候选为 `20260824-799adf9`，candidate build ID 为
...
自动化状态为 `candidate`；包内清单绑定同一源码、归档和 build ID，目标验收为
`manual-pending`，并以被拒绝的 `20260824-5318d4b` 为 superseded 记录。上一候选
`20260824-5318d4b` 的归档保持不可变，拒绝原因是真实 JS coverage 与 Adapter 重算不一致，
提交在后台任务启动前返回 `DETERMINISTIC_FORMAT_REVIEW_SNAPSHOT_MISMATCH`。
更早的 `20260824-2e7a3e6` 仍为 `rejected`：包内交付说明过期，且缺失大纲事实被写为 `null`。
...
历史候选源码 `ccad09fb1d8019da3a40f14610ab3bd75de1ec23` 曾删除批次级伪范围、加入精确模型能力表、脱敏空正文诊断和不可重试边界，并将旧工作流档案改为版本化一次迁移；但其跨运行时哈希契约仍有阻断缺陷，归档已拒绝。当前唯一自动化候选为 `20260824-799adf9`，目标 WPS、真实模型和 Issue #59 人工验收仍为 `manual-pending`。
...
| 麒麟构建来源与审计 | provenance、白名单组装、规则编译、正式插件、交付审计、v0.25.1 审计、runtime gate 和 lifecycle gate 均通过；lifecycle 终态为 `candidate` |
| Python 3.8 生命周期门禁 | 麒麟 V10 ARM64 / Python `3.8.10` 候选生命周期门禁通过；目标机验收仍独立待完成 |
| 归档校验 | 独立本地 checksum 与解包交付审计均通过；当前候选 SHA-256 为 `5f15e385358dcaea987e62f43cd2db1b943696372a7867449a986cdfc403f67c` |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 发布 `20260824-799adf9` 自动化候选，修复跨运行时格式审查 coverage/哈希契约；`20260824-5318d4b` 保持拒绝记录，Issue #59 仍为 `manual-pending` |
