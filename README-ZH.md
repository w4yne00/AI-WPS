...
| 交付状态 | 当前唯一自动化候选为 `20260824-f953c58`（`candidate`）；目标验收仍为 `manual-pending`（Issue #59） |
| 一期交付包 | `ai-wps-phase1-delivery-20260824-f953c58-v0251.tar.gz`，SHA-256：`833e71fcf5a6e2172c93e44cc3502d46e1ea89c5dc4abb77f658ac8c5ee77ee7`；源码提交：`f953c58312c8d3d42d3dccea402fccf55a3c7d53` |

### v0.25.1-alpha 交付状态

当前唯一自动化候选为 `20260824-f953c58`，candidate build ID 为
...
历史候选源码 `ccad09fb1d8019da3a40f14610ab3bd75de1ec23` 曾删除批次级伪范围、加入精确模型能力表、脱敏空正文诊断和不可重试边界，并将旧工作流档案改为版本化一次迁移；但其跨运行时哈希契约仍有阻断缺陷，归档已拒绝。当前唯一自动化候选为 `f953c58`，目标 WPS、真实模型和 Issue #59 人工验收仍为 `manual-pending`。
...
| 麒麟构建来源与审计 | `f953c58` 麒麟 Python 3.8.10 来源、白名单、规则/插件/审计/运行时/生命周期门禁均通过；终态为 `candidate` |
| Python 3.8 生命周期门禁 | `f953c58` 生命周期门禁通过，终态为 `candidate`；目标机验收仍独立待完成 |
| 归档校验 | 当前 `f953c58` SHA-256 为 `833e71fcf5a6e2172c93e44cc3502d46e1ea89c5dc4abb77f658ac8c5ee77ee7`；被拒绝 `afe109c` 归档保持原字节，SHA-256 为 `e3d4da0d1d8e1edc619d2101f45afb104ef8e3a6e5197e4b8e59b46513f78c6b` |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 发布唯一候选 `20260824-f953c58`，包含跨运行时 structure/format 哈希契约证据；`20260824-afe109c` 及更早候选均保持 `rejected`，Issue #59 仍为 `manual-pending` |
