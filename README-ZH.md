...
| 交付状态 | `20260822-e43dc8c` 已拒绝；替代源码 `d6842468922bb7c6b060964a3d6ec3248766e827` 已通过本地门禁，真实 Python 3.8 候选构建及目标机验收待执行 |
| 一期交付包 | 当前无可发布候选；不得继续分发 `20260822-e43dc8c` 及更早归档 |
...
替代源码 `d6842468922bb7c6b060964a3d6ec3248766e827` 删除了批次级伪范围，加入精确模型能力表、脱敏空正文诊断和不可重试边界，并将旧工作流档案改为版本化一次迁移。构建现在只接受完整 `HEAD`，并拒绝白名单源、格式规则源、构建工具或插件测试存在未提交差异。当前 Mac 没有 Python 3.8，且本次目标机 SSH 被执行额度限制阻断，因此尚未生成可发布归档。

验证摘要：

| 门禁 | 结果 |
| --- | --- |
| 当前源码 Adapter 全量测试 | `797 passed, 95 skipped` |
| Python 3.8 静态兼容扫描 | 通过，139 个文件 |
| 正式插件合同测试 | 17 项通过 |
| 新候选麒麟生命周期 | 未执行；当前无可发布候选 |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 拒绝 `20260822-e43dc8c` 及更早候选；源码 `d6842468922bb7c6b060964a3d6ec3248766e827` 修复格式审查批次索引、精确输出能力、PPT 空最终正文诊断和旧工作流迁移删除，并新增可复现构建门禁；真实 Python 3.8 候选构建与 Issue #59 目标机验收待执行 |
