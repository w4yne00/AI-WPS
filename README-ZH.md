...
| 交付状态 | 自动化候选构建，麒麟 V10/WPS 真机验收待执行 |
| 一期交付包 | 构建目标：`dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260811-v0231.tar.gz`；SHA-256 由同名 `.sha256` 文件记录 |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.23.1-alpha` | 修复运行时求值内置容器泛型注解导致的 Python 3.8 Adapter 导入失败；新增生产 Python 兼容性扫描和最终 tar 包运行门禁，由真实 Python 3.8 完整导入应用、启动 Uvicorn，并校验版本与关键配置接口。自动化通过只标记候选构建，麒麟 V10/WPS 真机验收仍独立执行 |
