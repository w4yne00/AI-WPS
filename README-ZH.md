...
| 交付状态 | Issue #153 汇总候选在自动化门禁通过后仅登记为 `candidate`；Issue #154 目标机验收保持 `manual-pending` |
| 基线状态 | `v0.25.3-alpha` 已依据 Issue #59 完成目标机验收，状态为 `target-accepted` |

`v0.26.0-preview.1` 汇总来源先行的 Excel“智能填写”、Word 自动/手工/疑似目录处理与格式位置问题组，以及九类任务的紧凑模型配置入口。自动化候选仍须 Issue #154 目标机验收；当前包不宣称已通过真实 WPS 或模型验收。`v0.25.3-alpha` 的 `target-accepted` 基线仍按 Issue #59 保留记录。
