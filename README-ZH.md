...
| 交付状态 | `20260824-ccad09f` 已通过麒麟 V10/Python 3.8 自动化构建与生命周期门禁，状态为 `candidate`；目标 WPS GUI 与真实模型人工验收待执行 |
| 一期交付包 | `dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260824-ccad09f-v0251.tar.gz`；SHA-256：`2c3f8b5004c40fb7271a6afe7e4c8a292acb227b9d3ec08afc7f6b561d413a02` |
...
新候选源码 `ccad09fb1d8019da3a40f14610ab3bd75de1ec23` 删除了批次级伪范围，加入精确模型能力表、脱敏空正文诊断和不可重试边界，并将旧工作流档案改为版本化一次迁移。运行时快照也会忽略已经完成迁移且已被用户删除的陈旧工作流字段，避免覆盖安装因缺失旧密钥进入恢复模式。构建只接受完整 `HEAD`，并拒绝白名单源、格式规则源、构建工具或插件测试存在未提交差异。

验证摘要：

| 门禁 | 结果 |
| --- | --- |
| 当前源码 Adapter 全量测试 | `798 passed, 95 skipped` |
| 麒麟 V10/Python 3.8 针对性测试 | `41 passed` |
| Python 3.8 交付兼容扫描 | 通过，82 个交付文件 |
| 正式插件合同测试 | 17 项通过 |
| 新候选麒麟生命周期 | 通过；含基线升级、全新安装、旧版本升级、已删除工作流覆盖安装和故障回滚 |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 候选 `20260824-ccad09f` 修复格式审查批次索引、精确输出能力、PPT 空最终正文诊断、旧工作流重复迁移及运行时快照误判；已通过麒麟 V10/Python 3.8 自动化构建和生命周期门禁，Issue #59 目标 WPS 与真实模型人工验收待执行 |
