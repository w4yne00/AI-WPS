...
| 交付状态 | 暂无可验收候选；`20260822-afc5470` 已因目标机复现阻断缺陷登记为 `rejected`，修复已进入源码，待重新构建 |
| 一期交付包 | 暂无当前候选；`20260822-afc5470` 仅保留为已拒绝的历史归档，不应继续安装验收 |
...
当前没有可安装验收的新候选包。重新构建前只能复核源码自动化结果；自动化仍不得宣称
`target-accepted`，真实 WPS 任务窗格和模型直连验收继续由 Issue #59 记录。
...
| WPS 原文范围协议精确回归 | 前端、后端和恶意值拒绝场景通过 |
| 正式插件合同测试 | 17 项通过 |
| 最近已拒绝候选的麒麟生命周期 | 曾通过自动门禁，但不代表当前源码已有候选包 |
| 目标 WPS GUI 和真实文档人工验收 | 等待新包，由 Issue #59 继续记录 |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 默认启用格式审查 v2并完善停用提示；连续拒绝四个目标机复现阻断缺陷的候选，当前源码已修复 WPS 不可用原文位置哨兵导致的索引错误，等待重新构建和 Issue #59 人工验收 |
