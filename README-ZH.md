...
| 交付状态 | `20260822-385a251` 候选已发布；麒麟 ARM/Python 3.8.10 自动化和生命周期门禁通过，真实 WPS 验收仍待 Issue #59 完成 |
| 一期交付包 | [`20260822-385a251 候选归档`](dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260822-385a251-v0251.tar.gz) · [`SHA-256`](dist-phase1-delivery-kit/ai-wps-phase1-delivery-20260822-385a251-v0251.tar.gz.sha256) |

### v0.25.1-alpha 交付状态

`20260822-afc5470` 包在目标机格式审查中复现“格式审查原文范围索引无效”，已与
`20260816`、`20260822-275099e`、`20260822-4ff1862` 一并登记为 `rejected`。
根因为 WPS 不可用的原文位置哨兵值经 JSON 序列化为 `null`，后端把这些可选值误判为非法索引，
从而在模型调用前中断。源码提交 `672875e` 已在前端丢弃非有限数值，并由后端忽略可选的 `null`
位置字段，同时保留对负数、字符串、布尔值和未知字段的拒绝。

基于记录提交 `385a251` 生成的新候选已在麒麟 ARM/Python 3.8.10 上通过包内依赖安装、
Adapter 导入、Uvicorn 启动、公开格式审查 API、安装升级、故障恢复和中断回滚门禁。
自动化结果仍只标记为 `candidate`，不得宣称 `target-accepted`；真实 WPS 任务窗格和模型直连验收继续由 Issue #59 记录。

验证摘要：

| 门禁 | 结果 |
| --- | --- |
| 当前源码 Adapter 全量测试 | `785 passed, 95 skipped` |
| 麒麟 ARM/Python 3.8.10 原文范围精确回归 | `2 passed` |
| 正式插件合同测试 | 17 项通过 |
| 新候选麒麟生命周期 | 通过；最终状态为 `candidate` |
| 目标 WPS GUI 和真实文档人工验收 | 由 Issue #59 继续记录 |
...
| 版本 | 更新点 |
| --- | --- |
| `v0.25.1-alpha` | 发布经麒麟 ARM/Python 3.8.10 完整生命周期门禁验证的 `20260822-385a251` 候选；修复 WPS 不可用原文位置哨兵导致的格式审查索引错误，Issue #59 继续负责真实 WPS 人工验收 |
