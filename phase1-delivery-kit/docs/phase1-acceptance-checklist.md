...
- [ ] `/health` 返回 `version=0.21.0-alpha`。
...
- [ ] 文档审查、智能分析、公式助手和智能总结共用默认 2 个运行槽位与 8 个 FIFO 排队位置。
...
- [ ] Word 四个功能、智能分析、公式助手和智能总结均可保存至少两个具名工作流档案，并可填写备注。
...
- [ ] 覆盖安装后，智能编写、智能仿写、文档审查、格式审查、智能分析、公式助手、智能总结仍命中原工作流档案。
