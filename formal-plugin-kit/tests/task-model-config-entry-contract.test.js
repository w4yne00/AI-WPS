...
function assertKeyboardContract() {
  const closed = { open: false, itemCount: 3, highlightedIndex: -1 };
  ...
  const up = helpers.reduceTaskModelConfigMenuKey({ open: true, itemCount: 3, highlightedIndex: 0 }, "ArrowUp");
  assert.strictEqual(up.highlightedIndex, 0);

  const select = helpers.reduceTaskModelConfigMenuKey({ open: true, itemCount: 3, highlightedIndex: 1 }, "Enter");
  ...
  const manage = helpers.reduceTaskModelConfigMenuKey({ open: true, itemCount: 3, highlightedIndex: 2 }, "Enter");
  assert.strictEqual(manage.action, "manage");
  assert.strictEqual(manage.open, false);
  assert.strictEqual(manage.restoreFocus, false);

  const escape = helpers.reduceTaskModelConfigMenuKey({ open: true, itemCount: 3, highlightedIndex: 1 }, "Escape");
...
function assertLayoutContract() {
  assert.ok(excelCssHead.includes(".task-model-config-trigger"), "compact trigger CSS must live before the shared tail");
  assert.ok(excelCssHead.includes("text-overflow: ellipsis"), "narrow panes must ellipsize the entry, not wrap mid-glyph");
  assert.ok(excelCssHead.includes("white-space: nowrap"), "entry label must not break onto a second line");
  assert.ok(excelCssHead.includes("overflow: hidden"), "entry must clip instead of overflowing 320px");
