...
assert.ok(excelRibbon.includes('label="表格助手"'));
assert.ok(excelRibbon.includes('id="btnAiExcelAnalysis"'));
assert.ok(excelRibbon.includes('label="智能分析"'));
assert.ok(excelRibbon.includes('id="btnAiExcelFormulaAssistant"'));
assert.ok(excelRibbon.includes('label="公式助手"'));
...
assert.ok(/\.\/taskpane-helpers\.js\?v=(0\.23\.1-alpha|1\.0\.0|0\.26\.0-preview\.1(?:-[0-9a-f]{7,12})?)/.test(html));
...
assert.ok(html.indexOf('结果预览') < html.indexOf('id="btn-view-history"'));
assert.ok(html.indexOf('id="result-output"') < html.indexOf('id="btn-copy-result"'));
...
assert.ok(js.includes("helpers.normalizeAdapterHealth"));
assert.ok(js.includes('healthState.status === "recovery"'));
...
assert.ok(js.includes('job.canCancel'));
assert.ok(js.includes('method: "DELETE"'));
assert.ok(js.includes('job.status === "cancelled"'));
assert.ok(js.includes('DOCUMENT_REVIEW_JOB_INTERRUPTED'));
assert.ok(js.includes('LONG_TASK_QUEUE_FULL'));
assert.ok(js.includes('DOCUMENT_REVIEW_AUTH_SNAPSHOT_FAILED'));
assert.ok(js.includes('adapter 已重启，原文档审查任务已中断'));
...
assert.ok(js.includes("renderDeterministicFormatReviewIssuePage"));
...
assert.ok(helperJs.includes("页面设置"));
assert.ok(helperJs.includes("其他格式项"));
...
assert.ok(js.includes('startWritingJob(state.latestDocumentPayload, "word.smart_write", "smartWrite")'));
assert.ok(js.includes('startWritingJob(state.latestDocumentPayload, "word.smart_imitation", "smartImitation")'));
assert.ok(js.includes("state.rewriteResult = setSmartWriteResult(result || {}, taskType)"));
...
assert.ok(/FRONTEND_BUILD_VERSION = "(0\.23\.1-alpha|1\.0\.0|0\.26\.0-preview\.1(?:-[0-9a-f]{7,12})?)"/.test(js));
...
assert.ok(js.includes("/provider/model-configurations"));
...
assert.ok(js.includes("确定性格式审查只读报告已生成"));
assert.ok(helperJs.includes("以下内容仅展示可由当前格式事实确认的问题"));
...
assert.ok(excelHtml.includes('id="task-model-config-trigger"'));
...
assert.ok(excelJs.includes("excelFormulaAssistant"));
assert.ok(excelJs.includes('byId("btn-open-settings").addEventListener("click"'));
assert.ok(excelJs.includes('state.currentMode === "settings" ? state.lastTaskMode : "settings"'));
assert.ok(excelJs.includes("helpers.normalizeAdapterHealth"));
assert.ok(excelJs.includes('healthState.status === "recovery"'));
...
assert.ok(excelJs.includes("/provider/model-configurations"));
...
assert.ok(excelJs.includes("/excel/formula-assistant/jobs"));
...
assert.ok(/build=(0\.23\.1-alpha|1\.0\.0|0\.26\.0-preview\.1(?:-[0-9a-f]{7,12})?)/.test(ribbonJs));
